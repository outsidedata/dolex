...
export class CleanRejected extends Error {
    constructor(reason) { super(reason); this.name = 'CleanRejected'; }
}
...
export function runPythonClean(code, values) {
    ...
    try {
        const cf = join(dir, 'clean_src.py');
        writeFileSync(cf, code); // model code — read, never interpolated
        const vf = join(dir, 'v.json');
        writeFileSync(vf, JSON.stringify(values));
        const rf = join(dir, 'runner.py');
        writeFileSync(rf, RUNNER); // trusted harness
        ...
        if (parsed.rejected)
            throw new CleanRejected(parsed.rejected); // screen/exec/shape rejection
