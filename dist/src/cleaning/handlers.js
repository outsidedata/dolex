...
const isoLike = (s) => /^\d{4}-\d{2}-\d{2}/.test(String(s));
...
const looksDate = (s) => /(\d{1,2}[\/\-.]\d{1,2}[\/\-.]\d{2,4})|([A-Za-z]{3,}\.?\s+\d{1,2},?\s+\d{4})|(\d{4}-\d{2}-\d{2})/.test(String(s));
export const HANDLERS = {
    ...
    'mixed-type': {
        ...
        accept: (pairs, f) => {
            const m = f.detail.match(/non-numeric value "([^"]+)"/);
            const lit = m ? m[1] : '';
    ...
    'sentinel-value': {
        ...
        accept: (pairs, f) => {
            const m = f.detail.match(/Extreme value ([\d.eE+-]+)/) || f.detail.match(/sentinel:\s*"([^"]+)"/);
            const lit = m ? m[1] : '';
    ...
    'dirty-categories': {
        ...
        accept: (pairs) => {
            const m = new Map(pairs.map(([o, c]) => [o, c]));
            ...
            for (const a of keys)
                for (const b of keys) {
                    if (a !== b && a.trim().toLowerCase() === b.trim().toLowerCase()) {
                        checked++;
                        if (m.get(a) === m.get(b) && !blank(m.get(a)))
                            merged = true;
                    }
                }
            const ok = checked === 0 ? pairs.every(([, c]) => typeof c === 'string' && c === c.trim().toLowerCase()) : merged;
...
export function validateFix(finding, code, sampleVals) {
    const handler = finding.column ? HANDLERS[finding.issue] : undefined;
    if (!handler)
        return { ok: false, summary: `no handler for issue "${finding.issue}"` };
    const { cleaned } = runPythonClean(code, sampleVals);
    const pairs = sampleVals.map((o, i) => [o, cleaned[i]]);
