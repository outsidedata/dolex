...
function looksLikeValue(token) {
    if (token === undefined)
        return false;
    if (token === '-')
        return true; // stdin sentinel is a legitimate value
    if (!token.startsWith('-'))
        return true;
