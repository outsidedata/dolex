 ...
 */
export async function emitChart({ args, body, html }) {
    ...
    if (isRefine) {
        ...
        if (body.alternatives?.length) {
            o.kv('switch to', body.alternatives.join(', '), log);
        }
    }
    else {
        ...
        if (body.compound)
            o.kv('layout', 'chart + companion data table', log);
    }
    ...
    if (outputs.html)
        o.kv('chart', outputs.html, log);
    if (outputs.png)
        o.kv('png', outputs.png, log);
