...
function loadCsvs(csvPaths) {
    ...
    for (const { filePath, tableName } of csvPaths) {
        ...
        for (const col of colNames) {
            const sampleStmt = db.prepare(`SELECT DISTINCT "${col}" FROM "${tableName}" WHERE "${col}" IS NOT NULL AND "${col}" != '' LIMIT ${SAMPLE_LIMIT}`);
            const samples = sampleStmt.all().map((r) => r[col]);
