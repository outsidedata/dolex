...
export function validateDsl(table, query) {
 ...
 */
export function validateDslWithJoins(schema, baseTableName, query) {
    ...
    if (query.orderBy) {
        ...
        for (const o of query.orderBy) {
            ...
            if (aliases.includes(o.field))
                continue;
            if (bucketAliases.includes(o.field))
                continue;
...
function validateJoinField(field, baseTableName, tableMap) {
    if (field.includes('.')) {
 ...
 */
function validateFieldRef(field, tableMap) {
    if (field.includes('.')) {
        const [tableName, colName] = field.split('.', 2);
        const table = tableMap.get(tableName);
        if (!table) {
...
function resolveField(field, tableMap) {
    if (field.includes('.')) {
        const [tableName, colName] = field.split('.', 2);
        const table = tableMap.get(tableName);
        if (!table)
    ...
    for (const [, tSchema] of tableMap) {
        ...
        if (col)
            return { table: tSchema, col };
    }
    return null;
}
// ─── BUCKET ALIAS HELPERS ────────────────────────────────────────────────────
function computeBucketAliases(query) {
    const aliases = [];
    if (query.groupBy) {
        for (const g of query.groupBy) {
            if (typeof g !== 'string' && g.bucket) {
...
function computeBaseOutputNames(query) {
    const names = [];
...
function validateWindowField(field, baseOutputNames) {
...
function validateFieldExists(field, colNames) {
...
function validatePercentileParam(percentile) {
    if (percentile == null) {
...
function validateAggregate(aggregate, col) {
    if (ANY_TYPE_AGGREGATES.includes(aggregate))
        return null;
