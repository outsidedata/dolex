 ...
 */
function mapMysqlType(mysqlType, columnName, uniqueCount, totalRows, extra) {
...
class MysqlConnectedSource {
    ...
    config;
    cachedSchema = null;
    constructor(id, name, pool, config) {
        this.id = id;
        this.name = name;
        this.pool = pool;
        this.config = config;
    ...
    async getSchema() {
        ...
        // Get all user tables
        const [tableRows] = await this.pool.query(`SELECT table_name FROM information_schema.tables
       WHERE table_schema = ? AND table_type = 'BASE TABLE'
       ORDER BY table_name`, [this.config.database]);
        const tables = [];
        for (const tableRow of tableRows) {
            const tableName = tableRow.table_name ?? tableRow.TABLE_NAME;
            // Get row count
            const [countRows] = await this.pool.query(`SELECT COUNT(*) as cnt FROM \`${tableName}\``);
            const rowCount = parseInt(countRows[0].cnt, 10);
            // Get columns from information_schema
            const [columnRows] = await this.pool.query(`SELECT column_name, data_type, is_nullable, column_default, extra
         FROM information_schema.columns
         WHERE table_schema = ? AND table_name = ?
         ORDER BY ordinal_position`, [this.config.database, tableName]);
            const columns = [];
            for (const colRow of columnRows) {
                ...
                let samples = [];
                try {
                    const [sampleRows] = await this.pool.query(`SELECT DISTINCT CAST(\`${colName}\` AS CHAR) AS val FROM \`${tableName}\` WHERE \`${colName}\` IS NOT NULL LIMIT ?`, [SAMPLE_LIMIT]);
                ...
                try {
                    const [statsRows] = await this.pool.query(`SELECT COUNT(DISTINCT \`${colName}\`) as uniq, COUNT(*) - COUNT(\`${colName}\`) as nulls, COUNT(*) as total FROM \`${tableName}\``);
                    uniqueCount = parseInt(statsRows[0].uniq, 10);
                    nullCount = parseInt(statsRows[0].nulls, 10);
                    totalCount = parseInt(statsRows[0].total, 10);
                }
                catch {
                    // Fallback
                }
    ...
    async getForeignKeys() {
       ...
       ORDER BY kcu.TABLE_NAME, kcu.COLUMN_NAME`, [this.config.database]);
        return rows.map((r) => ({
    ...
    async getSampleRows(tableName, count = 5) {
        try {
            const [rows] = await this.pool.query(`SELECT * FROM \`${tableName}\` ORDER BY RAND() LIMIT ?`, [count]);
            return rows;
        }
        catch {
            return [];
