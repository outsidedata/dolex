...
export function riskyDivisionTerms(sql) {
    const s = strip(sql);
    const out = [];
    for (const m of s.matchAll(TERM)) {
        const term = m[0];
        if (!term.includes('/'))
            continue; // pure multiplication can't truncate
        if (FLOAT_LIT.test(term) || CAST_REAL.test(term))
            continue; // float context → safe
...
export function divisionDenominators(sql) {
    const s = strip(sql);
    const re = new RegExp(`/\\s*(${OPERAND})`, 'g');
    const out = [];
    for (const m of s.matchAll(re)) {
        const denom = m[1].trim();
        if (/^NULLIF\s*\(/i.test(denom))
 ...
 */
export async function detectDivByZero(sql, columnHasZero) {
    const warnings = [];
    for (const { denom, columns } of divisionDenominators(sql)) {
        let zero = false;
        for (const col of columns) {
            if (await columnHasZero(col)) {
                zero = true;
                break;
            }
        }
        if (zero) {
            warnings.push({
                ...
                message: `Denominator "${denom}" contains zero values — SQLite returns NULL for x/0 (no error), and those rows silently drop out of aggregates. ` +
 ...
 */
export function detectBareAggregate(sql, isBaseColumn) {
    const s = strip(sql);
    if (/\bGROUP\s+BY\b/i.test(s))
        return []; // grouped → out of scope (conservative)
    const list = selectList(s);
    if (!list || !AGG.test(list))
        return [];
    ...
    if (aggs.length === 1 && (aggs[0] === 'MIN' || aggs[0] === 'MAX'))
        return [];
    ...
    return [{
            code: 'bare-aggregate',
            message: `Column${uniq.length > 1 ? 's' : ''} "${uniq.join('", "')}" ${uniq.length > 1 ? 'are' : 'is'} selected next to an aggregate with no GROUP BY — ` +
 ...
 */
export async function detectSqlFootguns(sql, columnType) {
    const warnings = [];
    for (const { term, columns } of riskyDivisionTerms(sql)) {
        ...
        let allInteger = true;
        for (const col of columns) {
            if ((await columnType(col)) !== 'integer') {
                allInteger = false;
                break;
            }
        }
