...
export function handleCleanColumn(deps) {
    return async (args) => {
        const ctx = await connectAndValidateTable(deps, args.sourceId, args.table);
        if (isTransformError(ctx))
            return ctx;
        if (!ctx.db)
            return errorResponse(`clean_column runs a Python clean() over CSV-backed rows. On a live ${ctx.source.type} source, express the fix as a derived column via transform_data using a native ${ctx.source.type === 'mongodb' ? 'aggregation ($set) expression' : 'SQL expression'} — non-destructive and server-side.`);
        const colNames = ctx.table.columns.map((c) => c.name);
        if (!colNames.includes(args.column))
            return errorResponse(`Column '${args.column}' not found. Available: [${colNames.join(', ')}]`);
        const newColumn = args.newColumn ?? `${args.column}_clean`;
        if (colNames.includes(newColumn))
            return errorResponse(`Column '${newColumn}' already exists. Pass a different newColumn or drop it first.`);
        if (!pythonAvailable())
            return errorResponse('clean_column requires python3 on PATH (not found). Install Python 3 to use this tool.');
        const rows = ctx.db.prepare(`SELECT rowid AS __rid, ${q(args.column)} AS v FROM ${q(args.table)}`).all();
        const rowids = rows.map((r) => r.__rid);
        ...
        let errors;
        try {
            ({ cleaned, errors } = runPythonClean(args.code, raw));
        }
        catch (e) {
            return errorResponse(`clean() failed to run: ${e?.message ?? String(e)}`);
        }
        ...
        if (!safety.ok)
            return errorResponse(`clean() rejected: ${safety.reason}. (nothing was applied)`);
        if (!args.apply) {
