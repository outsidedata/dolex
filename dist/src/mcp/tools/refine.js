...
export const refineInputSchema = z.object({
    ...
    filter: z.array(z.object({
        ...
        op: z.enum(['in', 'not_in', 'gt', 'gte', 'lt', 'lte', '=', '!='])
            .default('in')
            .transform(op => op === '=' ? 'in' : op === '!=' ? 'not_in' : op),
...
function applyAtomicParams(spec, args, stored) {
    const changes = [];
    const notes = [];
    ...
    if (args.xLabel !== undefined) {
        if (spec.encoding.x)
            spec.encoding.x.title = args.xLabel;
        changes.push(`X-axis label: "${args.xLabel}"`);
    }
    if (args.yLabel !== undefined) {
        if (spec.encoding.y)
            spec.encoding.y.title = args.yLabel;
    ...
    if (args.format !== undefined) {
        ...
        if (fmt) {
            if (spec.pattern === 'heatmap') {
                if (!spec.encoding.color)
                    spec.encoding.color = {};
                spec.encoding.color.format = fmt;
            }
...
function applyCompoundParams(spec, args, stored) {
    const changes = [];
    const notes = [];
    ...
    if (args.hideColumns !== undefined && args.hideColumns.length > 0) {
        ...
        if (tableView?.table?.columns) {
            ...
            tableView.table.columns = tableView.table.columns.filter(c => !hideSet.has(c.field.toLowerCase()) && !hideSet.has((c.title || '').toLowerCase()));
    ...
    if (args.title !== undefined) {
        ...
        const chartView = spec.views.find(v => v.type === 'chart');
        if (chartView?.chart)
            chartView.chart.title = args.title;
        changes.push(`Title: "${args.title}"`);
    }
    const chartView = spec.views.find(v => v.type === 'chart');
    if (chartView?.chart) {
        ...
        if (hasAtomicArgs) {
            ...
            const refinedChart = subResult.spec;
            chartView.chart = {
...
export function handleRefine() {
    return async (args) => {
        ...
        let spec = JSON.parse(JSON.stringify(stored.spec));
        if (args.switchPattern) {
            const altSpec = stored.alternatives.get(args.switchPattern);
            if (altSpec) {
                spec = JSON.parse(JSON.stringify(altSpec));
            }
            else {
                try {
        ...
        let result;
        if (isCompoundSpec(spec)) {
            result = applyCompoundParams(spec, args, stored);
        }
        else {
            result = applyAtomicParams(spec, args, stored);
        }
