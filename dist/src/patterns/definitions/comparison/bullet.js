...
export const bulletPattern = {
    ...
    name: 'Bullet Chart',
    category: 'comparison',
    description: 'Compact horizontal bar showing a primary measure against a target with qualitative range bands (poor/satisfactory/good). Stephen Few\'s replacement for gauges and meters.',
    bestFor: 'Dashboard KPI tracking with context: actual vs target vs qualitative assessment. Revenue vs quota, CPU usage vs threshold, NPS score vs benchmark.',
    notFor: 'Many categories without targets (use bar chart), composition or part-to-whole (use stacked bar or donut), time series (use line or sparkline).',
    dataRequirements: {
        minRows: 1,
        maxRows: 12,
        requiredColumns: [
    ...
    selectionRules: [
        ...
        },
        {
            ...
            weight: 70,
            matches: (ctx) => {
                return /\b(vs\.?\s*target|versus\s*target|actual\s*vs|against\s*(target|benchmark|quota|threshold|goal)|kpi\s*(track|dashboard|monitor)|performance\s*(vs|against|to)\s*(target|benchmark|quota))\b/i.test(ctx.intent);
            },
        },
        {
