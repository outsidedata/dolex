 ...
 */
export const connectedDotPlotPattern = {
    id: 'connected-dot-plot',
    name: 'Connected Dot Plot',
    category: 'comparison',
    description: 'Two dots per category connected by a line. The length of the connecting line encodes the gap between two values, making differences immediately visible.',
    bestFor: 'Comparing two metrics per category: budget vs actual, male vs female, 2020 vs 2024, target vs achieved.',
    notFor: 'Single metric (use bar), more than 2 metrics (use parallel coordinates or radar), time series with many points (use line).',
    dataRequirements: {
        minRows: 3,
        maxRows: 30,
        requiredColumns: [
    ...
    selectionRules: [
        ...
        },
        {
            ...
            weight: 50,
            matches: (ctx) => {
                return /\b(budget\s*vs|actual\s*vs|male\s*(and|vs)|female\s*(and|vs)|gap|both|two\s+metric|dual|paired|dumbbell)\b/i.test(ctx.intent);
            },
        },
        {
