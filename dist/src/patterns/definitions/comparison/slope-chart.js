 ...
 */
export const slopeChartPattern = {
    id: 'slope-chart',
    name: 'Slope Chart',
    category: 'comparison',
    description: 'Lines connecting two points per category, showing change between two states. The angle of each slope communicates direction and magnitude of change at a glance.',
    bestFor: 'Before/after comparisons, two-year trends, start vs end, A/B test results, policy change impacts.',
    notFor: 'More than 2 time points (use line chart), single time point (use bar), more than 15 categories (too cluttered).',
    dataRequirements: {
        minRows: 4,
        maxRows: 60,
        requiredColumns: [
    ...
    selectionRules: [
        {
            ...
            matches: (ctx) => {
                ...
                if (dateColumnCount >= 1) {
                    ...
                    if (dateCol && dateCol.uniqueCount === 2)
                        return true;
                }
                if (categoricalColumnCount >= 2) {
                    ...
                    if (secondaryCats.some((c) => c.uniqueCount === 2))
                        return true;
                }
                return false;
            },
        },
        {
            ...
            weight: 60,
            matches: (ctx) => {
                return /\b(before|after|start|end|then|now|initial|final|first|last|two\s*(year|point|period)|vs\.?\s|versus|a\s*\/\s*b|change\s+from|compared\s+to)\b/i.test(ctx.intent);
            },
        },
        {
