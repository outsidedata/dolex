 ...
 */
export const ridgelinePattern = {
    id: 'ridgeline',
    name: 'Ridgeline Plot',
    category: 'distribution',
    description: 'Overlapping density curves stacked vertically. Compact way to compare many distributions simultaneously, revealing patterns across groups.',
    bestFor: 'Comparing distributions across many groups (5-30): temperature by month, ratings by genre, scores by department.',
    notFor: 'Few groups under 4 (use violin), single variable (use histogram), very small sample sizes per group.',
    dataRequirements: {
        minRows: 50,
        requiredColumns: [
    ...
    selectionRules: [
        ...
        },
        {
            ...
            weight: 50,
            matches: (ctx) => {
                return /\b(ridgeline|joy\s*plot|all\s+(distribut|groups)|each\s+(month|year|group|category)|across\s+all|by\s+month|by\s+year)\b/i.test(ctx.intent);
            },
        },
        {
