 ...
 */
export const violinPattern = {
    id: 'violin',
    name: 'Violin Plot',
    category: 'distribution',
    description: 'Mirrored density curves for each group, showing the full distribution shape. Reveals bimodality, skewness, and distribution differences that box plots hide.',
    bestFor: 'Comparing distribution shapes across 2-8 groups. Finding bimodal distributions. Understanding where data is concentrated.',
    notFor: 'Single group (use histogram), many groups over 10 (use ridgeline), small sample sizes under 30 per group (density estimate unreliable).',
    dataRequirements: {
        minRows: 30,
        requiredColumns: [
    ...
    selectionRules: [
        ...
        },
        {
            ...
            weight: 50,
            matches: (ctx) => {
                return /\b(distribut|shape|compar.+(distribut|spread)|violin|density|bimodal|multimodal)\b/i.test(ctx.intent);
            },
        },
        {
            ...
            weight: 30,
            matches: (ctx) => {
                if (ctx.dataShape.categoryCount === 0)
                    return false;
