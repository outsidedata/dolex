 ...
 */
export const alluvialPattern = {
    id: 'alluvial',
    name: 'Alluvial Diagram',
    category: 'flow',
    description: 'Flows between categorical columns ordered left-to-right (often time). Shows how entities transition between states across stages: retention, churn, and movement.',
    bestFor: 'State transitions over time: customer segments changing over quarters, voting patterns across elections, career paths, cohort movement.',
    notFor: 'Non-temporal flows (use Sankey), simple composition (use stacked bar), very many categories per stage (too many flows).',
    dataRequirements: {
        minRows: 5,
        maxRows: 500,
        requiredColumns: [
    ...
    selectionRules: [
        ...
        },
        {
            ...
            weight: 60,
            matches: (ctx) => {
                return /\b(transition|movement|churn|retention|cohort|switch|migrat|convert|stage|phase|path|alluvial|from.+to.+over)\b/i.test(ctx.intent);
            },
        },
        {
            ...
            weight: 40,
            matches: (ctx) => {
                return (ctx.dataShape.categoricalColumnCount >= 3 &&
                    ctx.dataShape.numericColumnCount >= 1);
            },
        },
        {
