 ...
 */
export const chordPattern = {
    id: 'chord',
    name: 'Chord Diagram',
    category: 'flow',
    description: 'Circular layout with arcs for groups and curved chords connecting them. Chord width represents flow magnitude. Shows mutual relationships and overall connectivity.',
    bestFor: 'Bidirectional flows between groups: trade between countries, migration between regions, collaboration between departments, phone calls between cities.',
    notFor: 'Unidirectional flow (use Sankey), many groups over 12 (too dense), non-relational data.',
    dataRequirements: {
        minRows: 3,
        maxRows: 100,
        requiredColumns: [
    ...
    selectionRules: [
        {
            ...
            weight: 80,
            matches: (ctx) => {
                if (ctx.dataShape.categoricalColumnCount < 2 || ctx.dataShape.numericColumnCount < 1)
                    return false;
        ...
        },
        {
            ...
            weight: 60,
            matches: (ctx) => {
                return /\b(chord|mutual|bidirectional|between\s+groups|inter.*(department|region|country)|trade\s+between|migration\s+between|collaborat)\b/i.test(ctx.intent);
            },
        },
        {
