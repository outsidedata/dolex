 ...
 */
export const sankeyPattern = {
    id: 'sankey',
    name: 'Sankey Diagram',
    category: 'flow',
    description: 'Nodes connected by flows whose width represents quantity. Shows how a total distributes through a system: sources, intermediate steps, and destinations.',
    bestFor: 'Energy flow, budget allocation, user journey funnels, import/export trade, material flow, conversion pipelines.',
    notFor: 'Simple composition (use stacked bar or waffle), non-flow data, cyclical flows (Sankey requires directed acyclic graph).',
    dataRequirements: {
        minRows: 3,
        maxRows: 200,
        requiredColumns: [
    ...
    selectionRules: [
        {
            ...
            matches: (ctx) => {
                return (ctx.dataShape.categoricalColumnCount >= 2 &&
                    ...
                    /\b(from|source|origin)\b/i.test(ctx.columns
        ...
        },
        {
            ...
            weight: 70,
            matches: (ctx) => {
                return /\b(flow|sankey|funnel|path|journey|pipeline|from\s+.+\s+to|source.+target|allocation|distribut.+through)\b/i.test(ctx.intent);
            },
        },
        {
