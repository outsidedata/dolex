 ...
 * alone would reveal.
 */
export const connectedScatterPattern = {
    id: 'connected-scatter',
    name: 'Connected Scatterplot',
    category: 'relationship',
    description: 'Scatter plot with points connected in time order. Shows how the relationship between two variables evolves: loops indicate cycles, spirals indicate drift.',
    bestFor: 'Two metrics that change together over time: unemployment vs inflation (Phillips curve), GDP vs life expectancy, revenue vs customer count.',
    notFor: 'Static two-variable relationship (use regular scatter), single variable over time (use line), many data points (path becomes tangled).',
    dataRequirements: {
        minRows: 5,
        maxRows: 100,
        requiredColumns: [
    ...
    selectionRules: [
        ...
        },
        {
            ...
            weight: 60,
            matches: (ctx) => {
                return /\b(two\s+(metric|variable|measure)|evolv|over\s+time.+(vs|versus|and)|phillips|relationship.+chang|how\s+.+\s+and\s+.+\s+chang)\b/i.test(ctx.intent);
            },
        },
        {
