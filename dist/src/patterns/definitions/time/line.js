...
export const linePattern = {
    ...
    name: 'Line Chart',
    category: 'time',
    description: 'Points connected chronologically by lines. The standard for showing trends, cycles, seasonality, and change over continuous time.',
    bestFor: 'Single time series or comparing 2-5 series. Showing trends, growth patterns, seasonal effects.',
    notFor: 'More than 5 series (spaghetti — use small multiples), categorical data (use bar), distribution (use histogram).',
    dataRequirements: {
        minRows: 3,
        requiredColumns: [
    ...
    selectionRules: [
        ...
        },
        {
            ...
            weight: 40,
            matches: (ctx) => {
                return /\b(trend|over\s+time|time\s*series|month|year|quarter|growth|decline|trajectory|progress)\b/i.test(ctx.intent);
            },
        },
        {
