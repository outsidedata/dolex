 ...
 */
export function hasTimeSeriesColumn(columns) {
    return columns.some((col) => col.type === 'date' ||
 ...
 */
export function countCategories(data, columnName) {
 ...
 */
export function countSeries(data, columns) {
 ...
 */
export function hasNegativeValues(data, columns) {
 ...
 */
export function getValueRange(data, columns) {
    let min = Infinity;
    let max = -Infinity;
    const numericCols = columns.filter((c) => c.type === 'numeric');
    for (const col of numericCols) {
 ...
 */
export function detectHierarchy(data, columns) {
    const categoricalCols = columns.filter((c) => c.type === 'categorical');
    if (categoricalCols.length < 2)
        return false;
 ...
 */
export function buildMatchContext(data, columns, intent) {
    const categoricalCols = columns.filter((c) => c.type === 'categorical');
    const numericCols = columns.filter((c) => c.type === 'numeric');
 ...
 */
export function inferEncoding(columnNames, columns) {
