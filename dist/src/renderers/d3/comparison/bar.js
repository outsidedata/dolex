...
function renderHorizontalBar(g, data, originalData, encoding, config, dims, tooltip) {
    ...
    const allVals = data.map((d) => Number(d[xField]));
    ...
    g.selectAll('.bar')
        ...
        .join('rect')
        .attr('class', 'bar')
        .attr('y', (d) => yScale(d[yField]) + barYOffset)
        .attr('x', (d) => {
        const v = Number(d[xField]);
        return v >= 0 ? xScale(0) : xScale(v);
    })
        .attr('height', barHeight)
        .attr('width', (d) => Math.max(2, Math.abs(xScale(Number(d[xField])) - xScale(0))))
    ...
    if (showLabels) {
        g.selectAll('.bar-label')
            ...
            .text((d) => formatValue(Number(d[xField])));
        // Animation disabled for snappy rendering (was: .transition().delay(600).duration(200))
    }
}
function renderVerticalBar(g, data, originalData, encoding, config, dims, tooltip) {
    const xField = encoding.x.field;
    const yField = encoding.y.field;
    const categories = data.map((d) => d[xField]);
    const xScale = d3.scaleBand().domain(categories).range([0, dims.innerWidth]).padding(0.2);
    const allValsV = data.map((d) => Number(d[yField]));
    const minValV = d3.min(allValsV);
    const maxValV = d3.max(allValsV);
    const yScale = d3
        .scaleLinear()
    ...
    g.selectAll('.bar')
        ...
        .join('rect')
        .attr('class', 'bar')
        .attr('x', (d) => xScale(d[xField]) + barXOffset)
        .attr('y', (d) => {
        const v = Number(d[yField]);
        return v >= 0 ? yScale(v) : yScale(0);
    })
        .attr('width', barWidth)
        .attr('height', (d) => Math.max(2, Math.abs(yScale(0) - yScale(Number(d[yField])))))
