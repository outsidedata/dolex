...
export function renderBullet(container, spec) {
    ...
    const rangeFields = config.rangeFields;
    ...
    if (config.sortBy === 'value') {
        ...
        const order = config.sortOrder === 'ascending' ? 1 : -1;
        sortedData.sort((a, b) => order * String(a[metricField]).localeCompare(String(b[metricField]), undefined, { sensitivity: 'base', numeric: true }));
    }
    ...
    const colorEncoding = encoding.color || { field: metricField, type: 'nominal' };
