 ...
 */
function resolveCollisions(labels, minSpacing, minY, maxY) {
...
export function renderSlopeChart(container, spec) {
    ...
    const periods = config.periods || [...new Set(data.map((d) => d[timeField]))].slice(0, 2);
    ...
    categories.forEach((cat) => {
        ...
        data
            ...
            .forEach((d) => {
            grouped[cat][d[timeField]] = Number(d[valueField]);
        });
    });
