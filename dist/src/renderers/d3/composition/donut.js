...
export function renderDonut(container, spec) {
    ...
    const total = data.reduce((s, d) => s + (Math.abs(Number(d[valueField])) || 0), 0);
    // Compute min-visible threshold: 2% of max ensures extreme-range slices stay visible
    const rawValues = data.map((d) => Number(d[valueField]) || 0).filter((v) => v > 0);
    ...
    if (showLabels && items.length <= 8 && radius >= labelMinRadius) {
        ...
        g.selectAll('.donut-label')
            ...
            .attr('fill', (d) => contrastText(colorScale(d.data.category)))
            .attr('font-size', () => {
