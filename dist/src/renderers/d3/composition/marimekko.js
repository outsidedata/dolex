...
export function renderMarimekko(container, spec) {
    ...
    const secondaryKeys = [...new Set(data.map((d) => String(d[seriesField])))];
    let primaryKeys = [...new Set(data.map((d) => String(d[categoryField])))];
    const columnTotals = {};
    ...
    if (config.sortBy === 'value') {
        ...
        const order = config.sortOrder === 'ascending' ? 1 : -1;
        primaryKeys.sort((a, b) => order * a.localeCompare(b, undefined, { sensitivity: 'base', numeric: true }));
    }
    ...
    const grandTotal = data.reduce((s, d) => s + (Math.abs(Number(d[valueField])) || 0), 0);
    ...
    primaryKeys.forEach((pk, i) => {
        ...
        secondaryKeys.forEach((sk) => {
            const row = data.find((d) => String(d[categoryField]) === pk && String(d[seriesField]) === sk);
    ...
    g.selectAll('.mekko-segment')
        ...
        .attr('fill', (d) => colorScale(d.secondaryKey))
        .attr('stroke', DARK_BG)
        .attr('stroke-width', 1)
    ...
    if ((showLabels || showPercentages) && !isSmall) {
        g.selectAll('.mekko-label')
            ...
            .attr('fill', (d) => contrastText(colorScale(d.secondaryKey)))
            .attr('font-size', (d) => {
