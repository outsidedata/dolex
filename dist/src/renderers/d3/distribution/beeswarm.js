...
// ─── GROUPED BEESWARM ────────────────────────────────────────────────────────
function renderGroupedBeeswarm(g, data, valueField, categoryField, encoding, config, xScale, colorScale, baseRadius, opacity, dims, tooltip, isSmall) {
    ...
    g.selectAll('.dot')
        ...
        .attr('r', (p) => p.r)
        .attr('data-r', (p) => p.r)
        .attr('fill', (p) => colorScale(p.d[encoding.color?.field || categoryField]))
        .attr('opacity', opacity)
        .attr('stroke', DARK_BG)
...
function renderSingleBeeswarm(g, data, valueField, encoding, xScale, colorScale, baseRadius, opacity, dims, tooltip) {
    // ── Adaptive dot radius ──
