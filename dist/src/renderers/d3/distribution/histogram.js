 ...
 */
function buildBinColorFn(colorEncoding, xDomain) {
    if (!colorEncoding) {
...
// ─── PRE-BINNED ──────────────────────────────────────────────────────────────
function renderPreBinnedHistogram(container, spec) {
    const { data, encoding, config } = spec;
    const { svg, g, dims } = createSvg(container, spec, { right: 30 });
    const tooltip = createTooltip(container);
    const xMin = d3.min(data, (d) => d.binStart);
    const xMax = d3.max(data, (d) => d.binEnd);
    const xScale = d3
        .scaleLinear()
        .domain([xMin, xMax])
    ...
    g.selectAll('.bar')
        ...
        .attr('fill', (d) => binColor(d.binStart, d.binEnd))
        .attr('rx', 1)
    ...
    if (showLabels) {
        g.selectAll('.bar-label')
            ...
            .attr('x', (d) => xScale(d.binStart) + binWidth / 2)
            .attr('y', (d) => {
            ...
            .attr('pointer-events', 'none')
            .text((d) => formatValue(d.count));
    }
    // ── Stat lines ──
    drawStatLines(g, xScale, dims.innerHeight, config);
}
// ─── RAW DATA ────────────────────────────────────────────────────────────────
function renderRawHistogram(container, spec, xField) {
    const { data, encoding, config } = spec;
    ...
    g.selectAll('.bar-hover-target')
        ...
        .attr('y', 0)
        .attr('width', (d) => Math.max(xScale(d.x1) - xScale(d.x0), 1))
        .attr('height', dims.innerHeight)
        .attr('fill', 'transparent')
        .attr('cursor', 'pointer')
        .on('mouseover', function (event, d) {
    ...
    g.selectAll('.bar')
        ...
        .attr('fill', (d) => binColor(d.x0, d.x1))
        .attr('rx', 1)
        .attr('pointer-events', 'none');
    // ── Value labels ──
    const avgBinWidth = bins.length > 0
        ? Math.max(...bins.map((b) => xScale(b.x1) - xScale(b.x0)))
        : 0;
    ...
    if (showLabels) {
        g.selectAll('.bar-label')
            ...
            .attr('font-family', 'Inter, system-ui, sans-serif')
            .attr('pointer-events', 'none')
            .text((d) => d.length > 0 ? formatValue(d.length) : '');
    }
    // ── Stat lines ──
    const mean = config.mean ?? values.reduce((a, b) => a + b, 0) / values.length;
...
// ─── STAT LINES ──────────────────────────────────────────────────────────────
function drawStatLines(g, xScale, innerHeight, config) {
...
function drawStatLine(g, x, height, color, dashArray, label, labelY) {
    g.append('line')
        .attr('x1', x)
        .attr('y1', 0)
        .attr('x2', x)
