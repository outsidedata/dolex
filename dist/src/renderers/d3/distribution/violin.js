...
export function renderViolin(container, spec) {
    ...
    groups.forEach((group, gi) => {
        ...
        const maxDensity = d3.max(densityPoints, (d) => d.density) || 1;
        // Width scale: maps density to pixels (half-width of violin)
        const widthScale = (density) => (density / maxDensity) * maxHalfWidth;
        ...
        const violinArea = d3
            .area()
            .x0((d) => xCenter - widthScale(d.density))
            .x1((d) => xCenter + widthScale(d.density))
            .y((d) => yScale(d.value))
            .curve(d3.curveBasis);
        g.append('path')
            .datum(densityPoints)
        ...
        if (showQuartiles && groupValues.length >= 4) {
            ...
            [stats.q1, stats.q3].forEach((qVal) => {
                const qDensity = densityPoints.reduce((closest, pt) => Math.abs(pt.value - qVal) < Math.abs(closest.value - qVal) ? pt : closest, densityPoints[0]);
        ...
        if (showMedian && groupValues.length >= 1) {
            const stats = gd.stats;
            const medianDensity = densityPoints.reduce((closest, pt) => Math.abs(pt.value - stats.median) < Math.abs(closest.value - stats.median) ? pt : closest, densityPoints[0]);
