...
export function renderFunnel(container, spec) {
    ...
    const style = config.style || 'tapered';
    ...
    const stages = displayData.map((d, i) => {
    ...
    const maxValue = d3.max(stages, (s) => s.value) || 1;
    stages.forEach((s) => {
    ...
    const widthScale = (v) => Math.max(_widthScaleBase(v), v > 0 ? MIN_STAGE_WIDTH : 0);
    ...
    stages.forEach((stage, i) => {
        ...
        g.append('rect')
            ...
            g.selectAll('.funnel-stage').attr('opacity', (_, j) => j === i ? 1 : 0.3);
