...
function matchHint(input, patterns) {
    if (!input)
        return undefined;
...
function collectSuggestions(unmatched, candidates) {
    const results = [];
...
export function detectGeoScope(values, options) {
 ...
 */
export function buildGeoSpecConfig(geo) {
...
export function applyGeoScope(data, geoField, options) {
    const explicitRegion = options?.geoRegion;
    if (explicitRegion) {
