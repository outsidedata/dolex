import { createTooltip, showTooltip, hideTooltip, formatValue, parseDate, escapeHtml, TEXT_COLOR, TEXT_MUTED, DARK_BG, truncateLabel, styleAxis, getAdaptiveTickCount, truncateTitle, smartTruncateLabels, sequential, diverging, } from '../shared.js';
...
export function renderHorizonChart(container, spec) {
    ...
    const seriesNames = seriesField
        ? [...new Set(parsedData.map((d) => d[seriesField]))].sort()
        : ['all'];
    ...
    const allDates = parsedData.map((d) => d._date);
    ...
    seriesNames.forEach((name) => {
        const sd = isSingleSeries
            ? [...parsedData].sort((a, b) => a._date.getTime() - b._date.getTime())
            : parsedData
                .filter((d) => d[seriesField] === name)
                .sort((a, b) => a._date.getTime() - b._date.getTime());
        seriesDataMap.set(name, sd);
    });
    const uniqueTimeValues = new Set(parsedData.map((d) => d._date.getTime()));
    ...
    seriesNames.forEach((seriesName, idx) => {
        ...
        if (sd.length === 0)
            return;
        const values = sd.map((d) => d._value);
        const maxAbs = d3.max(values, (v) => Math.abs(v));
        if (maxAbs === 0)
            return;
    ...
    const xAxis = axisG.append('g')
        ...
        .tickPadding(8));
    styleAxis(xAxis);
    addCrosshairInteraction(svg, seriesNames, seriesDataMap, xScale, labelWidth, topMargin, rowH, rowGap, chartW, timeField, valueField, seriesField, tooltip, isSingleSeries);
}
function calculateLabelWidth(seriesNames) {
...
function pickBandColors(numBands, ramp) {
...
function addCrosshairInteraction(svg, seriesNames, seriesDataMap, xScale, labelWidth, topMargin, rowHeight, rowGap, chartW, timeField, valueField, seriesField, tooltip, isSingleSeries) {
