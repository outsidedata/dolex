...
export function renderSparklineGrid(container, spec) {
    ...
    const seriesGroups = d3.group(data, (d) => d[seriesField]);
    ...
    seriesNames.forEach((seriesName, idx) => {
        ...
        const values = seriesData.map((d) => Number(d[valueField]));
        ...
        if (seriesData.length <= 1) {
            ...
            const xScale = d3.scaleTime()
                .domain(d3.extent(seriesData, (d) => new Date(d[timeField])))
            ...
            const line = d3.line()
                ...
                .y((d) => yScale(Number(d[valueField])));
            g.append('path')
                .datum(seriesData)
                .attr('fill', 'none')
