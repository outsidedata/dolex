...
export function evaluateExpression(expr, rows, options) {
    ...
    if (rows.length > 0) {
        ...
        if (misquoted.length > 0) {
            ...
            throw new Error(`Misquoted column reference: ${list} ${misquoted.length === 1 ? 'is a string literal but matches a column name' : 'are string literals but match column names'}. ` +
    ...
    if (stats.rows > 0 && stats.nulls === stats.rows) {
        throw new Error(`Expression produced null for all ${stats.rows} rows — refusing to create an all-null column. ` +
...
function findMisquotedColumnLiterals(ast, columns) {
    ...
    function walkEqualityOperand(operand, sibling) {
        if (operand.type === 'string' && sibling.type === 'column')
            return; // legit value comparison
        walk(operand);
    }
    walk(ast);
    return [...hits];
