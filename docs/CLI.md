...
- `SourceManager` + the CSV connector — load CSVs into in-memory SQLite, run safe SQL
...
dolex <command> [arguments] [options]
  ...
  sources <list|add|remove>  Manage the persistent CSV registry (~/.dolex/sources.json)
...
lexicographic comparisons), **mixed-type** columns, **all-null / constant**
columns, **identical/leaked** columns, **duplicate rows**, **missing-value
sentinels** (`N/A`, `-999`, …), numeric **outliers**, and **quoting footguns**
(names needing backticks, year-as-number). **Exits non-zero on HIGH-severity
findings**, so scripts/agents can gate on it.
...
### analyze / describe / patterns / sources
...
src/cli/
  ...
  commands/
    visualize.ts  refine.ts  query.ts  analyze.ts  describe.ts  patterns.ts  sources.ts
    transform.ts  columns.ts  drop.ts  mcp.ts  help.ts
```
