...
export function buildAnalysisPlan(
  ...
  const steps = candidates
    .filter(step => {
      if (seen.has(step.category)) return false;
  ...
  for (const col of classified) {
    if (col.role === 'time') roleCounts.time++;
    else if (col.role === 'dimension' || col.role === 'hierarchy') roleCounts.dimension++;
    else if (col.role === 'measure') roleCounts.measure++;
  }

  const parts: string[] = [];
  if (roleCounts.time > 0) parts.push(plural(roleCounts.time, 'time column'));
  if (roleCounts.dimension > 0) parts.push(plural(roleCounts.dimension, 'dimension'));
  if (roleCounts.measure > 0) parts.push(plural(roleCounts.measure, 'measure'));
