...
export function auditColumns(table: string, columns: DataColumn[], rowCount: number): QualityFinding[] {
  ...
  for (const col of columns) {
    ...
    if (col.type === 'numeric' && col.coercedNonNumeric && col.coercedNonNumeric.count >= COERCED_SENTINEL_MIN) {
      ...
      add('high', col.name, 'mixed-type',
        `${count} cells hold a non-numeric value "${token}" that was coerced to NULL because the column is numeric.`,
        `It is a string sentinel, not a number. \`WHERE ${esc(col.name)} = '${token}'\` matches NOTHING (the value is gone) — count its rows on a TEXT column instead, e.g. another draft_* field, and treat it as missing when aggregating ${col.name}.`);
    }
    ...
    if ((col.type === 'categorical' || col.type === 'text') && typeProbe.length >= 2) {
      ...
      if (nt) {
        add('high', col.name, nt.issue,
          `Values are ${nt.label} stored as text (e.g. ${nt.examples.join(', ')}).`,
          `${nt.harm} Convert before aggregating: ${nt.fix(esc(col.name))}.`);
      }
    }
    ...
    if ((col.type === 'categorical' || col.type === 'text') && (col.topValues?.length ?? 0) >= 2 && !looksFreeText) {
      ...
      for (const raw of catPool) {
        ...
        if (prev !== undefined && prev !== raw) { collision = [prev, raw]; break; }
    ...
    if (col.type === 'numeric' && col.stats) {
      ...
      if (iqr > 0) {
        if (min < p25 - 3 * iqr) extremes.push(min);
        if (max > p75 + 3 * iqr) extremes.push(max);
      }
      const sentinels = extremes.filter(isSentinelNumber);
      for (const s of sentinels) {
        add('high', col.name, 'sentinel-value',
          `Extreme value ${fmt(s)} is a classic missing-value sentinel, not real data (a far outlier vs p25–p75 ${fmt(p25)}…${fmt(p75)}).`,
...
function isSentinelNumber(v: number): boolean {
  if (!Number.isFinite(v)) return false;
  return NUMERIC_SENTINELS.has(v) || /^9{3,}$/.test(String(Math.abs(v)));
}

interface NumericTextKind {
  issue: string;
  label: string;
  examples: string[];
  harm: string;
  fix: (col: string) => string;
}
...
function classifyNumericText(probe: string[]): NumericTextKind | null {
...
// ── Table-level (query-based) checks: duplicate rows + identical columns ──
// Moved here from the CLI `check` command so the loop and the CLI share ONE
// auditor (single source of truth). Driven through an injected query function so
// it works over any engine (CLI's openTarget, the loop's SourceManager).
...
export async function tableLevelChecks(
  t: { name: string; columns: DataColumn[]; rowCount: number },
  query: AuditQueryFn,
  ...
  if (t.rowCount > 0 && t.rowCount <= MAX_DUP_ROWS_SCAN) {
    const res = await query(
      `SELECT (SELECT COUNT(*) FROM ${esc(t.name)}) - (SELECT COUNT(*) FROM (SELECT DISTINCT * FROM ${esc(t.name)})) AS dups`,
    );
    const dups = res.ok && res.rows && res.rows[0] ? Number((res.rows[0] as any).dups) : NaN;
  ...
  const groups = new Map<string, DataColumn[]>();
  ...
  for (const group of groups.values()) {
    ...
    for (let i = 0; i < group.length; i++) {
      for (let j = i + 1; j < group.length; j++) {
        if (comparisons++ >= MAX_IDENTICAL_PAIRS) break;
        ...
        const diff = res.ok && res.rows && res.rows[0] ? Number((res.rows[0] as any).diff) : NaN;
...
export async function auditDataset(
  tables: { name: string; columns: DataColumn[]; rowCount: number }[],
  query: AuditQueryFn,
): Promise<QualityFinding[]> {
  const findings: QualityFinding[] = [];
...
const SEV_RANK: Record<QualitySeverity, number> = { high: 0, medium: 1, low: 2 };
...
export function formatAuditForPrompt(findings: QualityFinding[], maxLines = 5): string {
  ...
  const byIssue = new Map<string, QualityFinding[]>();
  for (const f of worthy) {
    const arr = byIssue.get(f.issue);
    if (arr) arr.push(f);
    else byIssue.set(f.issue, [f]);
  }
  ...
  for (const [, group] of byIssue) {
    ...
    if (group.length === 1) {
      ...
      const ids = group.map((g) => g.column).filter((c): c is string => Boolean(c));
  ...
  if (issuesTotal > shown) lines.push(`• …and ${issuesTotal - shown} more issue type(s).`);
  return lines.join('\n');
}
