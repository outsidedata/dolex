...
export async function buildCleanManifest(
  rawPath: string, author: CleanAuthor, opts: { tries?: number; onLog?: (s: string) => void; createdBy?: string } = {},
): Promise<CleanManifest> {
  const log = opts.onLog ?? (() => {});
  const tries = opts.tries ?? 3;
  const rows = Papa.parse<Record<string, string>>(fs.readFileSync(rawPath, 'utf8'), { header: true, skipEmptyLines: true }).data;
  const mgr = new SourceManager(); const add = await mgr.add('ds', { type: 'csv', path: rawPath }); const sid = add.entry!.id; await mgr.connect(sid);
  const tables = (await mgr.getSchema(sid)).schema!.tables as any;
  const findings = await auditDataset(tables, (s: string) => mgr.querySql(sid, s));
  const manifest: CleanManifest = { dataset: path.basename(rawPath), createdBy: opts.createdBy ?? 'injected-author', createdAt: '', fixes: [] };

  for (const f of findings) {
    const handler = f.column ? HANDLERS[f.issue] : undefined;
    if (!handler) continue;
    const task = handler.task(f); if (!task) continue;
    if (manifest.fixes.some((x) => x.column === f.column)) continue; // one fix per column
    ...
    if (sentinel !== undefined && !sampleVals.includes(sentinel)) sampleVals.push(sentinel);
    const samples = sampleVals.slice(0, 4);
    let feedback: string | undefined, stored = false;
    for (let attempt = 1; attempt <= tries; attempt++) {
      let code: string;
      try { code = await author({ column: f.column!, issue: f.issue, task, samples, feedback }); }
      catch (e: any) { log(`  ${f.column} attempt ${attempt}: author error ${e?.message}`); continue; }
      let acc;
      try { acc = validateFix(f, code, sampleVals); }
      catch (e: any) { acc = { ok: false, summary: 'python error', fail: String(e?.message || e).slice(0, 80) }; }
      log(`  ${f.column} (${f.issue}) attempt ${attempt}: ${acc.summary} → ${acc.ok ? 'VALID' : 'FAIL ' + (acc.fail ?? '')}`);
      if (acc.ok) { manifest.fixes.push({ column: f.column!, issue: f.issue, task, pythonCode: code, validated: true, summary: acc.summary }); stored = true; break; }
      feedback = acc.fail;
    }
    if (!stored) log(`  ${f.column}: no valid fix after ${tries} tries — skipped (never store an unvalidated fix)`);
  }
  return manifest;
}

/** Replay validated fixes over a CSV (NON-destructive: keep <col>_raw). NO model. */
export function applyManifest(rawPath: string, manifest: CleanManifest): { rows: Record<string, string>[]; fields: string[] } {
  const parsed = Papa.parse<Record<string, string>>(fs.readFileSync(rawPath, 'utf8'), { header: true, skipEmptyLines: true });
...
export async function cleanDataset(
  rawPath: string, author: CleanAuthor, opts: { outDir?: string; onLog?: (s: string) => void; createdBy?: string } = {},
): Promise<{ cleanedPath: string; manifestPath: string; manifest: CleanManifest }> {
