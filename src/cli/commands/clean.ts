...
import {
  pythonAvailable, runPythonClean, cleanStats, safetyVerdict, previewSample,
  applyManifest, type CleanManifest, type FixRecord,
} from '../../cleaning/index.js';

const BOOLEANS = ['apply', 'help'];
const ALIASES: Record<string, string> = { h: 'help', c: 'column' };

export async function cleanCommand(argv: string[]): Promise<number> {
  const args = parseArgs(argv, { booleans: BOOLEANS, aliases: ALIASES });
  if (bool(args, 'help')) { printHelp(); return 0; }
  const target = args._[0];
  if (!target) { o.fail('Usage: dolex clean <csv> --column <col> --code-file <fix.py> [--apply]'); return 1; }
  if (!pythonAvailable()) { o.fail('dolex clean requires python3 on PATH (not found).'); return 1; }

  const manifestArg = str(args, 'apply-manifest');
  if (manifestArg !== undefined) return replay(target, manifestArg);

  const column = str(args, 'column');
  const codeFile = str(args, 'code-file');
  if (!column || !codeFile) { o.fail('Pass --column <col> and --code-file <fix.py> (the caller authors the clean()).'); return 1; }
  ...
  try {
    ...
    if (!q.ok) { o.fail(`could not read column "${column}": ${q.error}`); return 1; }
    const raw = (q.rows ?? []).map((r: any) => (r.v === null || r.v === '' ? null : String(r.v)));
    let cleaned: (string | null)[]; let errors: number;
    try { ({ cleaned, errors } = runPythonClean(code, raw)); }
    catch (e: any) { o.fail(`clean() rejected: ${e?.message ?? String(e)} (nothing written)`); return 1; }
    ...
    for (const s of sample.slice(0, 10)) o.out(`  ${JSON.stringify(s.before)} → ${JSON.stringify(s.after)}`);
    if (!safety.ok) { o.fail(`rejected: ${safety.reason} (nothing written)`); return 1; }

    if (!bool(args, 'apply')) { o.hint('Preview only — pass --apply to write <base>.clean.csv (non-destructive).'); return 0; }
    // --apply: materialize via a single-fix manifest over the source CSV file.
    const csvPath = resolve(target);
    const fix: FixRecord = { column, issue: str(args, 'issue') ?? 'manual', task: 'caller-authored', pythonCode: code, validated: true, summary: `${stats.changed} changed` };
    const manifest: CleanManifest = { dataset: basename(csvPath), createdBy: 'dolex clean', createdAt: '', fixes: [fix] };
...
function replay(target: string, manifestPath: string): number {
  const csvPath = resolve(target);
  const mPath = manifestPath || join(dirname(csvPath), `${basename(csvPath, '.csv')}.cleanfix.json`);
  const manifest = JSON.parse(readFileSync(resolve(mPath), 'utf8')) as CleanManifest;
  const { rows, fields } = applyManifest(csvPath, manifest);
