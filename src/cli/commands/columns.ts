...
export async function columnsCommand(argv: string[]): Promise<number> {
  ...
  try {
    ...
    for (const col of body.derived_columns as { column: string; expr: string; type: string; partitionBy?: string }[]) {
    ...
    if (body.working_columns.length > 0) {
      ...
      for (const col of body.working_columns as { column: string; expr: string; type: string }[]) {
