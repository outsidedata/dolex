...
const ALIASES: Record<string, string> = { h: 'help', c: 'category' };

export async function patternsCommand(argv: string[]): Promise<number> {
  ...
  if (id) {
    ...
    o.out(`  ${p.description}`);
    if (p.bestFor) o.kv('best for', p.bestFor);
    if (p.notFor) o.kv('not for', p.notFor);
    const req = p.dataRequirements ?? {};
    const reqParts: string[] = [];
    if (req.minRows != null) reqParts.push(`≥${req.minRows} rows`);
    if (req.maxRows != null) reqParts.push(`≤${req.maxRows} rows`);
    if (req.requiresTimeSeries) reqParts.push('time series');
    if (req.requiresHierarchy) reqParts.push('hierarchy');
    if (reqParts.length) o.kv('requires', reqParts.join(', '));
    o.out('');
  ...
  if (bool(args, 'json')) {
    o.out(
      JSON.stringify(
        all.map((p) => ({ id: p.id, name: p.name, category: p.category, bestFor: p.bestFor })),
        null,
        2,
      ),
    );
