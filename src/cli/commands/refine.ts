...
export async function refineCommand(argv: string[]): Promise<number> {
  ...
  let specId = '';
  let hydrated: ReturnType<typeof loadSpec> = null;
  ...
  let refineArgs: ReturnType<typeof refineInputSchema.parse>;
  try {
  ...
  if (newStored) persistSpec(body.specId, newStored);

  return emitChart({ args, body, html });
}

/** Translate CLI flags into the refineInputSchema shape (only set keys present). */
function buildRefineObject(specId: string, args: ReturnType<typeof parseArgs>): Record<string, any> {
  const obj: Record<string, any> = { specId };

  const sort = str(args, 'sort');
  if (sort !== undefined) obj.sort = parseSort(sort);

  const limit = num(args, 'limit');
  if (limit !== undefined) obj.limit = limit;

  const filter = str(args, 'filter');
  if (filter !== undefined) obj.filter = parseFilters(filter);

  if (bool(args, 'flip')) obj.flip = true;
  ...
  if (bool(args, 'remove-table')) obj.removeTable = true;

  const hide = list(args, 'hide-columns');
  if (hide !== undefined) obj.hideColumns = hide;
 ...
 */
export function parseSort(raw: string): { field?: string; direction: 'asc' | 'desc' } | null {
  const trimmed = raw.trim();
  const lower = trimmed.toLowerCase();
  if (lower === 'none' || lower === 'clear') return null;
  const dir = (s: string): 'asc' | 'desc' => (s.trim().toLowerCase() === 'asc' ? 'asc' : 'desc');
  const [a, b] = trimmed.split(':').map((s) => s.trim());
  if (b) return { field: a, direction: dir(b) };
  if (a.toLowerCase() === 'asc' || a.toLowerCase() === 'desc') return { direction: dir(a) };
 ...
 */
export function parseFilters(raw: string): { field: string; op: string; values: (string | number)[] }[] {
  const trimmed = raw.trim().toLowerCase();
  if (trimmed === 'clear' || trimmed === 'none' || trimmed === '') return [];

  const clauses: { field: string; op: string; values: (string | number)[] }[] = [];
