...
export async function sourcesCommand(argv: string[]): Promise<number> {
  ...
  switch (sub) {
    case 'list': {
      ...
      o.out(
        o.table(
          ...
          entries.map((e) => ({ name: e.name, id: e.id, type: e.type, location: describeLocation(e.config) })),
        ),
      );
      return 0;
    }
    ...
    case 'test': {
      ...
      if (!idOrName) { o.fail('Usage: dolex sources test <name|id>'); return 1; }
      const r = await manager.testSource(idOrName);
      if (bool(args, 'json')) { o.out(JSON.stringify({ source: idOrName, ...r }, null, 2)); return r.ok ? 0 : 1; }
      if (r.ok) { o.success(`"${idOrName}" is reachable ✓`); return 0; }
      ...
      if (r.kind && nextStep[r.kind]) o.hint(nextStep[r.kind]);
      return 1;
    }

    case 'update': {
      const idOrName = args._[1];
      if (!idOrName) { o.fail('Usage: dolex sources update <name|id> [--host … --port … --database … --user … --password-env … --uri …]'); return 1; }
      const patch: Record<string, unknown> = {};
      for (const k of ['uri', 'host', 'database', 'user', 'password', 'password-env', 'schema'] as const) {
        const v = str(args, k); if (v !== undefined) patch[k === 'uri' ? 'connectionString' : k === 'password-env' ? 'passwordEnv' : k] = v;
      }
      const p = num(args, 'port'); if (p !== undefined) patch.port = p;
      if (Object.keys(patch).length === 0) { o.fail('Nothing to update. Pass at least one of --host/--port/--database/--user/--password-env/--uri/--schema.'); return 1; }
      if (patch.password) o.warn('--password is stored in plaintext. Prefer --password-env <VAR>.');
      const res = await manager.update(idOrName, patch as any, { verify: bool(args, 'verify') });
      if (!res.ok) { o.fail(res.error ?? 'Failed to update source.'); return 1; }
      o.success(`Updated "${idOrName}"`);
      if (res.verified === false) o.warn(`Saved, but still can't connect: ${res.warning}. Test again:  dolex sources test ${idOrName}`);
      return 0;
    }
...
const hostPortDb = (connStr: string | undefined, host: string | undefined, port: number | undefined, database: string | undefined): string =>
  connStr ?? [host, port].filter(Boolean).join(':') + (database ? `/${database}` : '');

function describeLocation(config: DataSourceConfig): string {
  switch (config.type) {
    case 'csv':
      return config.path ?? '';
    case 'postgres':
