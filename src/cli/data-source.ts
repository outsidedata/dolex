 ...
 */
async function loadManagerModule(): Promise<typeof import('../connectors/manager.js')> {
  try {
    ...
    if (/better-sqlite3|bindings|NODE_MODULE_VERSION|did not self-register|invalid ELF/i.test(msg)) {
      throw new Error(
        "Dolex's data engine (better-sqlite3, a native module) failed to load.\n" +
...
export async function persistentManager(): Promise<SourceManager> {
 ...
 */
export async function openTarget(target: string, opts: { table?: string } = {}): Promise<OpenedSource> {
  const { SourceManager } = await loadManagerModule();
  ...
  if (opts.table) {
    ...
    if (!found) {
      throw new Error(
        `Table '${opts.table}' not found. Available: ${tables.map((t) => t.name).join(', ')}`,
      );
    }
  ...
  return {
    ...
    tables,
    defaultTable,
    query: (sql: string, maxRows?: number) => manager.querySql(sourceId, sql, maxRows),
 ...
 */
export async function readInlineRows(
