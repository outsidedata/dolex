...
export function defaultChartPath(specId: string): string {
  return join(dolexHome(), 'charts', `chart-${specId}.html`);
}

/** Write HTML to a path (creating parent dirs). Returns the absolute path. */
export function writeHtmlFile(html: string, outPath: string): string {
...
export async function renderPng(
  ...
  const { chromium } = await importOptional<{ chromium: any }>('playwright', 'png');
