 ...
 */
function buildClient(config: PostgresSourceConfig): pg.Client {
  return new Client({
    host: config.host,
    port: config.port,
...
class PostgresConnectedSource implements ConnectedSource {
  id: string;
  name: string;
      ...
      for (const colRow of columnsResult.rows) {
        ...
        try {
          const sampleResult = await this.client.query(
            `SELECT DISTINCT "${colName}"::text AS val FROM "${tableName}" WHERE "${colName}" IS NOT NULL LIMIT ${SAMPLE_LIMIT}`
          );
          samples = sampleResult.rows.map((r: any) => r.val);
        } catch {
...
export const postgresConnector: DataConnector = {
  type: 'postgres',

  async test(
    config: DataSourceConfig
  ): Promise<{ ok: boolean; error?: string }> {
    const pgConfig = config as PostgresSourceConfig;
