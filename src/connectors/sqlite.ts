...
class SqliteConnectedSource implements ConnectedSource {
  id: string;
  name: string;
  readonly type: DataSourceType = 'sqlite';
  private db: Database.Database;
  private dbPath: string;
  private cachedSchema: DataSchema | null = null;

  constructor(id: string, name: string, db: Database.Database, dbPath: string) {
    this.id = id;
    this.name = name;
    this.db = db;
    this.dbPath = dbPath;
  ...
  async getSchema(): Promise<DataSchema> {
    ...
    for (const tableName of tableNames) {
      ...
      const pragmaRows = this.db
        ...
        .all() as { cid: number; name: string; type: string; notnull: number; dflt_value: any; pk: number }[];

      const columns: DataColumn[] = [];
      for (const pragma of pragmaRows) {
        // Sample values
        const sampleStmt = this.db.prepare(
          `SELECT DISTINCT "${pragma.name}" FROM "${tableName}" WHERE "${pragma.name}" IS NOT NULL LIMIT ${SAMPLE_LIMIT}`
        );
        const samples = sampleStmt
          .all()
          .map((r: any) => String(r[pragma.name]));

        // Stats
        const statsStmt = this.db.prepare(
          `SELECT COUNT(DISTINCT "${pragma.name}") as cnt, COUNT(*) - COUNT("${pragma.name}") as nulls, COUNT(*) as total FROM "${tableName}"`
        );
        const stats = statsStmt.get() as { cnt: number; nulls: number; total: number };

        const type = mapSqliteType(
          pragma.type,
          pragma.name,
          samples,
          stats.cnt,
...
export const sqliteConnector: DataConnector = {
  type: 'sqlite',

  async test(config: DataSourceConfig): Promise<{ ok: boolean; error?: string }> {
    const sqliteConfig = config as SqliteSourceConfig;
