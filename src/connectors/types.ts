...
import type { DataSourceType, DataSourceConfig, DataSchema, DataColumn } from '../types.js';
 ...
 */
export function registerDerivedColumn(schema: DataSchema, table: string, column: string): void {
  const t = schema.tables.find((tt) => tt.name === table);
  if (t && !t.columns.find((cc) => cc.name === column)) {
    t.columns.push({ name: column, type: 'text', sampleValues: [], uniqueCount: 0, nullCount: 0, totalCount: t.rowCount, layer: 'derived' } as DataColumn);
  }
}
...
export interface DataConnector {
  ...
  test(config: DataSourceConfig): Promise<{ ok: boolean; error?: string }>;
}
