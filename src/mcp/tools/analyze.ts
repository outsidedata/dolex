...
export function handleAnalyzeSource(deps: { sourceManager: any }) {
  return async (args: z.infer<typeof analyzeSourceInputSchema>) => {
    ...
    const targetTable = args.table
      ? tables.find((t: any) => t.name === args.table)
      : tables[0];

    if (!targetTable) {
      const available = tables.map((t: any) => t.name).join(', ');
    ...
    try {
      ...
      if (err instanceof PlannerUnsupportedSourceError) return errorResponse(err.message);
      throw err;
    }
    ...
    const plan = buildAnalysisPlan(
      targetTable.columns,
      targetTable.name,
      schemaResult.schema.source?.name ?? args.sourceId,
      args.maxSteps ?? 6,
      dialect,
    );
