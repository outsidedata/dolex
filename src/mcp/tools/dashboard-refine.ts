...
interface RefinementAction {
  type: 'add_view' | 'remove_view' | 'modify_view' | 'layout' | 'filter' | 'theme' | 'swap' | 'title' | 'unknown';
 ...
 */
function splitClauses(refinement: string): string[] {
...
function parseClause(clause: string, views: DashboardViewSpec[]): RefinementAction | null {
 ...
 */
function parseRefinements(refinement: string, views: DashboardViewSpec[]): RefinementAction[] {
...
export function handleRefineDashboard(deps: { sourceManager: any }) {
  return async (args: z.infer<typeof refineDashboardInputSchema>) => {
    const spec = JSON.parse(JSON.stringify(args.currentSpec)) as DashboardSpec;
    const refinement = args.refinement;
    const changes: string[] = [];

    const actions = parseRefinements(refinement, spec.views as DashboardViewSpec[]);
      ...
      switch (action.type) {
        ...
        case 'layout': {
          ...
          if (action.details.viewId && action.details.colSpan) {
            if (!spec.layout.viewSizes) spec.layout.viewSizes = {};
        ...
        case 'add_view': {
          ...
          const newView: DashboardViewSpec = {
            id: newId,
        ...
        case 'filter': {
          if (action.details.action === 'add' && action.details.field) {
            if (!spec.globalFilters) spec.globalFilters = [];
        ...
        case 'modify_view': {
          ...
          for (const view of targetViews) {
            ...
            if (action.details.colorPreferences) {
              if (!view.colorPreferences) view.colorPreferences = {};
    ...
    const viewResult = await executeDashboardViews(
      spec.views as DashboardViewSpec[],
      spec.sourceId,
      spec.table,
      deps.sourceManager,
    );
    if (isViewExecutionError(viewResult)) return viewResult;

    const html = buildDashboardHtml(spec, viewResult.viewData);

    return htmlResponse(
      { dashboardSpec: spec, changes, viewReasonings: viewResult.viewReasonings },
      html,
    );
  };
}
