...
export function handleDropColumns(deps: { sourceManager: any }) {
  return async (args: z.infer<typeof dropColumnsSchema>) => {
    ...
    if (!ctx.db) return errorResponse(`drop_columns manages CSV-side derived columns. On a live ${ctx.source.type} source, derived columns are session-local — end the session (or overwrite via transform_data) to discard them.`);
