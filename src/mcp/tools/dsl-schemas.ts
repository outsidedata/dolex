...
const FILTER_SHORTHAND_OPS: Record<string, string> = {
  equals: '=',
  not_equals: '!=',
  gt: '>',
  gte: '>=',
...
function normalizeFieldAlias(obj: Record<string, unknown>): void {
...
function normalizeFilterInput(raw: unknown): unknown {
...
function normalizeOrderByInput(raw: unknown): unknown {
...
function normalizeSelectInput(raw: unknown): unknown {
  if (typeof raw === 'string' || !raw || typeof raw !== 'object') return raw;
  const obj = { ...(raw as Record<string, unknown>) };

  normalizeFieldAlias(obj);

  // Already canonical aggregate or window
  if (obj.aggregate || obj.window) return obj;
...
function normalizeGroupByInput(raw: unknown): unknown {
  if (typeof raw === 'string' || !raw || typeof raw !== 'object') return raw;
  const obj = { ...(raw as Record<string, unknown>) };
  normalizeFieldAlias(obj);
  return obj;
...
function normalizeArrayOrSingle(val: unknown, normalizeItem?: (item: unknown) => unknown): unknown {
  if (val === undefined || val === null) return val;
  const arr = Array.isArray(val) ? val : [val];
  return normalizeItem ? arr.map(normalizeItem) : arr;
}

export function normalizeDslQueryInput(raw: unknown): unknown {
  if (!raw || typeof raw !== 'object') return raw;
  const obj = raw as Record<string, unknown>;
  const result = { ...obj };
