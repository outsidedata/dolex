...
export function handlePromoteColumns(deps: { sourceManager: any }) {
  return async (args: z.infer<typeof promoteColumnsSchema>) => {
    ...
    if (!ctx.db) return errorResponse(`promote_columns persists derived columns to a CSV-side manifest. On a live ${ctx.source.type} source, derived columns are session-local (created with transform_data) and are not persisted — nothing to promote.`);
