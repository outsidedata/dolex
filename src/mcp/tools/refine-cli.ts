...
import type { VisualizationSpec, CompoundVisualizationSpec } from '../../types.js';
  ...
  return async (args: z.infer<typeof refineCliInputSchema>) => {
    ...
    if (outputHtml) {
      ...
      const pattern = isCompoundSpec(updatedStored.spec)
        ? (updatedStored.spec as any).views?.find((v: any) => v.type === 'chart')?.chart?.pattern
        : (updatedStored.spec as VisualizationSpec).pattern;
