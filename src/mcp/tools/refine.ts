...
const PALETTE_LOOKUP: [RegExp, ColorPaletteName, string][] = [
...
function matchPalette(text: string): [ColorPaletteName, string] | null {
  const lower = text.toLowerCase();
  for (const [regex, palette, label] of PALETTE_LOOKUP) {
    if (regex.test(lower)) return [palette, label];
  }
  return null;
}
...
function parseHighlight(refinement: string, lower: string): {
  values: string[];
  color?: string;
  mutedOpacity?: number;
} | null {
  // "highlight X and Y" or "highlight X, Y, Z"
  const highlightMatch = lower.match(
    /highlight\s+(.+?)(?:\s+in\s+(#[0-9a-f]{3,8}|\w+))?$/
  );
  ...
  const result: { values: string[]; color?: string; mutedOpacity?: number } = { values };
  if (highlightMatch[2]) result.color = highlightMatch[2];

  return result;
}

/** Parse muted opacity from refinement */
function parseMutedOpacity(lower: string): number | null {
  ...
  if (match2) return parseInt(match2[1], 10) / 100;
...
function applyRefinement(spec: VisualizationSpec, refinement: string): RefineOutput {
  const changes: string[] = [];
  const updated = JSON.parse(JSON.stringify(spec)) as VisualizationSpec;
  const lower = refinement.toLowerCase();

  // Sort refinements
  if (lower.includes('sort') && lower.includes('desc')) {
    if (updated.encoding.x) updated.encoding.x.sort = 'descending';
    if (updated.encoding.y) updated.encoding.y.sort = 'descending';
    changes.push('Applied descending sort');
  } else if (lower.includes('sort') && lower.includes('asc')) {
    if (updated.encoding.x) updated.encoding.x.sort = 'ascending';
    if (updated.encoding.y) updated.encoding.y.sort = 'ascending';
  ...
  if (lower.includes('color') || lower.includes('palette') || lower.includes('scheme') ||
    ...
    if (paletteResult) {
      if (!updated.encoding.color) updated.encoding.color = {};
...
function applyCompoundRefinement(
  spec: CompoundVisualizationSpec,
  refinement: string
): RefineOutput {
  const changes: string[] = [];
  const updated = JSON.parse(JSON.stringify(spec)) as CompoundVisualizationSpec;
  const lower = refinement.toLowerCase();
  ...
  if (hideMatch) {
    ...
    if (tableView?.table?.columns) {
      tableView.table.columns = tableView.table.columns.filter(
        c => c.field.toLowerCase() !== colName && (c.title || '').toLowerCase() !== colName
      );
  ...
  if (titleMatch) {
    ...
    const chartView = updated.views.find(v => v.type === 'chart');
    if (chartView?.chart) chartView.chart.title = updated.title;
    changes.push(`Changed title to "${updated.title}"`);
  }
  ...
  if (changes.length === 0) {
    ...
    if (chartView?.chart) {
      ...
      const refinedChart = chartResult.spec as VisualizationSpec;
      chartView.chart = {
...
export function handleRefine() {
  return async (args: z.infer<typeof refineInputSchema>) => {
    const start = Date.now();
    let spec: VisualizationSpec | CompoundVisualizationSpec;
    ...
    const result = isCompoundSpec(spec)
      ? applyCompoundRefinement(spec as CompoundVisualizationSpec, args.refinement)
      : applyRefinement(spec as VisualizationSpec, args.refinement);
