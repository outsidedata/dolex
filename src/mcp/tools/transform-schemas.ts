...
export const transformDataSchema = transformDataBaseSchema.refine(
  (d) => (d.create && d.expr) || d.transforms,
  { message: 'Provide either (create + expr) for single-column mode, or transforms array for batch mode' }
).refine(
  (d) => !((d.create || d.expr) && d.transforms),
  { message: 'Cannot use both single-column and batch mode' }
);
