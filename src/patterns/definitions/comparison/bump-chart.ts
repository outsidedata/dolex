...
export const bumpChartPattern: VisualizationPattern = {
  id: 'bump-chart',
  name: 'Bump Chart',
  category: 'comparison',
  description:
    'Tracks rank positions of entities over time or ordered categories. Lines connect rankings, and crossings show when one entity overtakes another.',
  bestFor:
    'Ranking changes: team standings over seasons, product market share rank, country GDP ranking over decades, leaderboard progression.',
  notFor:
    'Absolute values (use line chart), single time point (use bar), too many entities (>12 gets unreadable), non-ordinal x-axis.',
  ...
  selectionRules: [
    {
      ...
      weight: 90,
      matches: (ctx) => {
        return /\b(rank|ranking|position|standing|leaderboard|leader|place|moved up|moved down|overtake|overtook)\b/i.test(
          ctx.intent
        );
      },
    },
  ...
  generateSpec: (data, columns, options) => {
    ...
    const rankedData: Record<string, any>[] = [];
