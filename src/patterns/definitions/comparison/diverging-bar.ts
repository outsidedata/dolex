...
export const divergingBarPattern: VisualizationPattern = {
  id: 'diverging-bar',
  name: 'Diverging Bar Chart',
  category: 'comparison',
  description:
    'Horizontal bars extending left and right from a central baseline. Shows positive and negative deviations clearly.',
  bestFor:
    'Data with positive and negative values, profit/loss, above/below average, sentiment scores, change from baseline.',
  ...
  selectionRules: [
    {
      ...
      matches: (ctx) => {
        return (
          ctx.dataShape.hasNegativeValues &&
    ...
    },
    {
      ...
      weight: 50,
      matches: (ctx) => {
        return /\b(differ|change|deviat|baseline|target|above|below|gain|loss|profit|deficit|surplus|sentiment|positive|negative)\b/i.test(
          ctx.intent
        );
      },
    },
