...
export const metricPattern: VisualizationPattern = {
  id: 'metric',
  name: 'Metric',
  category: 'composition',
  description:
    'Big-number KPI cards with optional delta/trend indicators. Auto-abbreviates large values and arranges metrics in a responsive CSS Grid.',
  bestFor:
    'Dashboard headlines, KPI summaries, scoreboards, single-stat callouts, executive overviews with a handful of key numbers.',
  notFor:
    'Many data points (use bar or table), time series trends (use sparkline-grid or line), comparison across many categories.',
  ...
  generateSpec: (data, columns, options) => {
    ...
    const numericCols = columns.filter((c) => {
      if (c === labelCol || c === valueCol) return false;
      const sample = data[0]?.[c];
      return typeof sample === 'number' || !isNaN(Number(sample));
    });
