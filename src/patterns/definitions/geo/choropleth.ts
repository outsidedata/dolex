...
export const choroplethPattern: VisualizationPattern = {
  id: 'choropleth',
  name: 'Choropleth Map',
  category: 'geo',
  description:
    'Geographic regions shaded by data values. Darker colors indicate higher values, instantly revealing spatial patterns.',
  bestFor:
    'Country-level data: GDP, population, election results, disease rates, temperature. Any numeric metric with a geographic key.',
  notFor:
    'Point data (use proportional symbol), routes/trajectories, non-geographic categories, data with no geographic key.',
  ...
  selectionRules: [
    {
      ...
      weight: 85,
      matches: (ctx) => {
        return /\b(map|geographic|choropleth|country|countries|state\s+map|province|spatial|world\s+map|heat\s*map.*countr)\b/i.test(
          ctx.intent
        );
      },
    },
