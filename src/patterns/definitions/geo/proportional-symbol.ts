...
export const proportionalSymbolPattern: VisualizationPattern = {
  id: 'proportional-symbol',
  name: 'Proportional Symbol Map',
  category: 'geo',
  description:
    'Circles placed on a map, sized proportionally to data values. Avoids the area-distortion bias of choropleths.',
  bestFor:
    'City-level data: population, revenue by location, earthquake magnitudes, facility capacity. Point-based metrics.',
  notFor:
    'Area-based metrics (use choropleth), data without lat/lon or city names, non-geographic data.',
  ...
  selectionRules: [
    {
      ...
      weight: 80,
      matches: (ctx) => {
        return /\b(bubble\s*map|proportional|symbol\s*map|city|cities|point|points\s+on\s+map|location|locations)\b/i.test(
          ctx.intent
        );
      },
    },
    {
      ...
      weight: 60,
      matches: (ctx) => {
        return ctx.columns.some((col) =>
          /\b(lat|latitude|lon|longitude|lng|coord)/i.test(col.name)
        );
      },
    },
    {
