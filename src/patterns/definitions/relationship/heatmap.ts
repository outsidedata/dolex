...
export const heatmapPattern: VisualizationPattern = {
  id: 'heatmap',
  name: 'Heatmap',
  category: 'relationship',
  description:
    'A matrix grid where rows and columns are categories and cell color encodes a numeric value. Reveals patterns, clusters, and outliers in two-dimensional categorical data.',
  bestFor:
    'Correlation matrices, cross-tabulation, confusion matrices, frequency tables, skill matrices, attendance grids, any 2D categorical × numeric data.',
  notFor:
    'Time series (use calendar-heatmap or line), single categorical axis (use bar), continuous axes (use scatter), more than ~50 categories per axis (unreadable).',
  ...
  selectionRules: [
    {
      ...
      weight: 80,
      matches: (ctx) => {
        return /\b(heatmap|heat\s*map|matrix|correlation|cross[- ]?tab|confusion|crosstab|grid\s*chart)\b/i.test(
          ctx.intent
        );
      },
    },
    ...
    },
    {
      ...
      weight: 40,
      matches: (ctx) => {
        if (ctx.dataShape.categoricalColumnCount < 2) return false;
  ...
  generateSpec: (data, columns, options) => {
    const catCols = columns.filter(c => {
      ...
      const numeric = vals.every((v: any) => !isNaN(Number(v)) && v !== '' && v !== null);
      return !numeric;
    });
    ...
    if (catCols.length === 1 && numCols.length > 1) {
      ...
      const melted: Record<string, any>[] = [];
