...
export const parallelCoordinatesPattern: VisualizationPattern = {
  id: 'parallel-coordinates',
  name: 'Parallel Coordinates',
  category: 'relationship',
  description:
    'Multiple parallel vertical axes, one per variable. Each entity is a polyline crossing all axes at its values. Reveals multivariate patterns, clusters, and tradeoffs.',
  bestFor:
    'Comparing entities across 4+ numeric dimensions: car specs (mpg, hp, weight, price), player stats, product features, survey multi-question analysis.',
  notFor:
    'Few variables under 3 (use scatter or bar), very many rows over 200 (lines overlap — filter first), non-numeric data.',
  ...
  selectionRules: [
    ...
    },
    {
      ...
      weight: 60,
      matches: (ctx) => {
        return /\b(multivariat|multi.dimension|across\s+all|all\s+(metrics|variables|dimensions)|profile|parallel|specs|specifications)\b/i.test(
          ctx.intent
        );
      },
    },
    {
      ...
      matches: (ctx) => {
        return (
          ctx.dataShape.numericColumnCount >= 3 &&
  ...
  generateSpec: (data, columns, options) => {
    ...
    let groupCol: string | undefined = options?.groupField;
