...
export const scatterPattern: VisualizationPattern = {
  id: 'scatter',
  name: 'Scatter Plot',
  category: 'relationship',
  description:
    'Each point represents one observation, positioned by two numeric variables. Reveals correlations, clusters, outliers, and the shape of relationships.',
  bestFor:
    'Exploring the relationship between two numeric variables: height vs weight, price vs quantity, score vs study hours.',
  notFor:
    'Categorical data (use bar), time series (use line unless comparing two metrics), single variable (use histogram).',
  ...
  selectionRules: [
    ...
    },
    {
      ...
      weight: 50,
      matches: (ctx) => {
        return /\b(correlat|relationship|scatter|association|regress|predict|x\s+vs\s+y|versus)\b/i.test(
          ctx.intent
        );
      },
    },
      ...
      encoding: {
        ...
        size: sizeCol
          ? {
              ...
              title: sizeCol,
              range: options?.sizeRange ?? [3, 20] as [number, number],
            }
          : undefined,
      },
      config: {
