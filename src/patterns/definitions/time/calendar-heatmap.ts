...
export const calendarHeatmapPattern: VisualizationPattern = {
  id: 'calendar-heatmap',
  name: 'Calendar Heatmap',
  category: 'time',
  description:
    'Values mapped onto a calendar grid (weeks as columns, days as rows). Color intensity encodes magnitude. Reveals day-of-week effects, seasonal patterns, and anomalous days.',
  bestFor:
    'Daily data over months or years: website traffic, sales, commits, exercise, temperature. Finding day-of-week and seasonal patterns.',
  notFor:
    'Non-daily data (weekly, monthly — use line), very short time spans (use bar), non-temporal data.',
  ...
  selectionRules: [
    {
      ...
      weight: 80,
      matches: (ctx) => {
        if (!ctx.dataShape.hasTimeSeries || ctx.dataShape.dateColumnCount === 0) return false;
    ...
    },
    {
      ...
      weight: 60,
      matches: (ctx) => {
        return /\b(daily|calendar|day.of.week|weekday|weekend|heatmap|every\s+day|github|activity|contribution)\b/i.test(
          ctx.intent
        );
      },
    },
