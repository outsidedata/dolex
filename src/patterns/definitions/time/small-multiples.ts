...
export const smallMultiplesPattern: VisualizationPattern = {
  id: 'small-multiples',
  name: 'Small Multiples',
  category: 'time',
  description:
    'A grid of small charts, one per series, all sharing the same axes. Avoids the spaghetti problem of overlaying many time series while enabling comparison.',
  bestFor:
    'Comparing 5-25 time series: stock prices of multiple companies, temperature by city, sales by product line.',
  notFor:
    'Fewer than 4 series (use regular line chart), more than 30 series (use sparkline grid), single series.',
  ...
  selectionRules: [
    ...
    },
    {
      ...
      weight: 50,
      matches: (ctx) => {
        return /\b(small\s*multiple|facet|each\s+(country|city|product|company|region|team)|all\s+.*\s+over\s+time|individual\s+trend)\b/i.test(
          ctx.intent
        );
      },
    },
