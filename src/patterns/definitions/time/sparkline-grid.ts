...
export const sparklineGridPattern: VisualizationPattern = {
  id: 'sparkline-grid',
  name: 'Sparkline Grid',
  category: 'time',
  description:
    'Dense grid of minimal line charts (sparklines), one per series. No axes or labels — just the shape of each trend. Designed for scanning many series at once.',
  bestFor:
    'Dashboard overview of 10-100 series: KPI dashboard, all product metrics, all regional trends. Finding which series stand out.',
  notFor:
    'Precise value reading (no axes), few series (use line chart or small multiples), non-temporal data.',
  ...
  selectionRules: [
    ...
    },
    {
      ...
      weight: 50,
      matches: (ctx) => {
        return /\b(overview|dashboard|all\s+\w+|summary|scan|sparkline|compact|every\s+(product|metric|region))\b/i.test(
          ctx.intent
        );
      },
    },
  ...
  generateSpec: (data, columns, options) => {
    ...
    const spec: VisualizationSpec = {
      ...
      config: {
        ...
        gridCols,
        seriesMeta,
        cellWidth: options?.cellWidth ?? undefined,
        cellHeight: options?.cellHeight ?? undefined,
