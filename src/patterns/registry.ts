...
class PatternRegistry {
  private patterns: Map<string, VisualizationPattern> = new Map();
   ...
   */
  getCategories(): { category: PatternCategory; count: number; patterns: string[] }[] {
    const categories = new Map<PatternCategory, string[]>();
...
function isCompatible(
  ...
  if (req.minRows !== undefined && opts.rowCount < req.minRows) return false;
  if (req.maxRows !== undefined && opts.rowCount > req.maxRows * 2) return false; // Soft limit: allow 2x over

  if (req.requiresTimeSeries && !opts.hasTimeSeries && opts.dateColumnCount === 0) return false;
  if (req.requiresHierarchy && !opts.hasHierarchy) return false;
  ...
  if (req.requiredColumns) {
    for (const colReq of req.requiredColumns) {
      ...
      if (available < colReq.count) return false;
    }
  }
...
const ALL_PATTERNS: VisualizationPattern[] = [
  // Comparison
  barPattern,
  divergingBarPattern,
  slopeChartPattern,
