...
export function renderConnectedDotPlot(container: HTMLElement, spec: VisualizationSpec): void {
  ...
  if (config.sortBy === 'value' || config.sortBy === 'gap') {
    ...
    const order = config.sortOrder === 'ascending' ? 1 : -1;
    sortedData.sort((a, b) => {
      return order * String(a[categoryField]).localeCompare(
        String(b[categoryField]), undefined, { sensitivity: 'base', numeric: true }
      );
    });
  }
  ...
  const minVal = d3.min(allValues) as number;
  const maxVal = d3.max(allValues) as number;
  ...
  const yAxis = g
    ...
    .call(
      d3
        ...
        .tickPadding(10)
        .tickFormat((d: string) => {
          if (d.length > 30) return d.slice(0, 29) + '\u2026';
          return d;
        })
    );
...
function css(cat: string): string {
  return cat.replace(/[^a-zA-Z0-9]/g, '_');
}

function drawDot(
  g: any, cx: number, cy: number, r: number,
...
function drawLabel(g: any, cx: number, cy: number, value: number, bgColor: string): void {
  g.append('text')
    .attr('x', cx)
    .attr('y', cy)
    .attr('text-anchor', 'middle')
