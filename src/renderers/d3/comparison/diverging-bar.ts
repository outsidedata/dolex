...
export function renderDivergingBar(container: HTMLElement, spec: VisualizationSpec): void {
  const { encoding, data, config } = spec;
  const orientation = (config as any)?.orientation === 'vertical' ? 'vertical' : 'horizontal';
...
function renderHorizontal(
  ...
  g.selectAll('.bar-hover-target')
    ...
    .attr('y', (d: any) => yScale(String(d[xField])))
    .attr('x', 0)
  ...
  g.selectAll('.bar')
    ...
    .join('rect')
    .attr('class', 'bar')
    .attr('y', (d: any) => yScale(String(d[xField])) + barYOffset)
    .attr('height', barHeight)
    .attr('x', (d: any) => {
      const val = Number(d[yField]);
      return val >= 0 ? midX : xScale(val);
    })
    .attr('width', (d: any) => Math.max(2, Math.abs(xScale(Number(d[yField])) - midX)))
    .attr('fill', (d: any) => (Number(d[yField]) >= 0 ? '#10b981' : '#ef4444'))
    .attr('rx', 3)
  ...
  if (showLabels) {
    g.selectAll('.bar-label')
      ...
      .attr('y', (d: any) => yScale(String(d[xField]))! + yScale.bandwidth() / 2)
      .attr('x', (d: any) => {
        const val = Number(d[yField]);
      ...
      .attr('text-anchor', (d: any) => {
        ...
        if (val >= 0) return barWidth > 50 ? 'end' : 'start';
...
function renderVertical(
  ...
  g.selectAll('.bar-hover-target')
    ...
    .join('rect')
    .attr('class', 'bar-hover-target')
    .attr('x', (d: any) => xScale(String(d[xField])))
    .attr('y', 0)
  ...
  g.selectAll('.bar')
    ...
    .join('rect')
    .attr('class', 'bar')
    .attr('x', (d: any) => xScale(String(d[xField])) + barXOffset)
    .attr('width', clampedBarWidth)
    .attr('y', (d: any) => {
      const val = Number(d[yField]);
      return val >= 0 ? yScale(val) : midY;
    })
    .attr('height', (d: any) => Math.max(2, Math.abs(yScale(Number(d[yField])) - midY)))
    .attr('fill', (d: any) => (Number(d[yField]) >= 0 ? '#10b981' : '#ef4444'))
    .attr('rx', 3)
    .attr('pointer-events', 'none');
  ...
  if (showLabels) {
    g.selectAll('.bar-label')
      ...
      .join('text')
      .attr('class', 'bar-label')
      .attr('x', (d: any) => xScale(String(d[xField]))! + xScale.bandwidth() / 2)
      .attr('y', (d: any) => {
        const val = Number(d[yField]);
