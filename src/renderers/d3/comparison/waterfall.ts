...
export function renderWaterfall(container: HTMLElement, spec: VisualizationSpec): void {
  ...
  const totalColumns: Set<string> = new Set();
  if (Array.isArray(config.totalColumns)) {
    for (const t of config.totalColumns) {
      if (t === 'first') totalColumns.add(String(data[0]?.[categoryField]));
      else if (t === 'last') totalColumns.add(String(data[data.length - 1]?.[categoryField]));
      else totalColumns.add(String(t));
    }
  } else {
    if (data.length >= 2) {
  ...
  const xAxis = g.append('g')
    ...
    .call(d3.axisBottom(xScale).tickSize(0).tickPadding(8)
      .tickFormat((d: string) => truncateLabel(d, 20)));
  styleAxis(xAxis);
  ...
  g.selectAll('.bar-hover-target')
    ...
    .attr('x', (d: WaterfallItem) => xScale(d.label))
    .attr('y', 0)
  ...
  g.selectAll('.bar')
    ...
    .join('rect')
    .attr('class', 'bar')
    .attr('data-label', (d: WaterfallItem) => d.label)
    .attr('x', (d: WaterfallItem) => (xScale(d.label) ?? 0) + wBarOffset)
    .attr('y', (d: WaterfallItem) => yScale(Math.max(d.start, d.end)))
    .attr('width', wBarWidth)
    .attr('height', (d: WaterfallItem) => Math.max(2, Math.abs(yScale(d.start) - yScale(d.end))))
    .attr('fill', (d: WaterfallItem) => {
  ...
  if (showLabels) {
    g.selectAll('.bar-label')
      ...
      .attr('x', (d: WaterfallItem) => (xScale(d.label) ?? 0) + xScale.bandwidth() / 2)
