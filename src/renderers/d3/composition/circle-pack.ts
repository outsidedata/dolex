...
export function renderCirclePack(container: HTMLElement, spec: VisualizationSpec): void {
  ...
  if (root.value === 0) {
    root.each((node: any) => {
      if (!node.children) node.value = 1;
    });
    root.sum((d: any) => d.children ? 0 : 1);
  }
  ...
  if (config.showLabels !== false) {
    leaves.forEach((leaf: any, i: number) => {
      if (leaf.r < 15) return; // Too small for any label
