 ...
 */
export function createMinVisibleClamp(
  ...
  return (v: number) => (v > 0 && v < minVisible ? minVisible : v);
}
 ...
 */
export function buildHierarchy(opts: BuildHierarchyOptions): any {
  const {
    data,
    valueField,
    categoryField,
  ...
  const root = d3.hierarchy(hierarchy).sum((d: any) => d.value);
  if (sort) {
 ...
 */
export function getTopAncestor(d: any): any {
  let node = d;
 ...
 */
export function getAncestorPath(d: any): string {
  const parts: string[] = [];
  let node = d;
 ...
 */
export function getAncestorChain(d: any): Set<any> {
