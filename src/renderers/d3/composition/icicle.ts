...
export function renderIcicle(container: HTMLElement, spec: VisualizationSpec): void {
  const { config, encoding, data } = spec;

  const levelFields: string[] = config.levelFields || [];
  const valueField: string = config.valueField || '';
  const orientation: string = config.orientation || 'horizontal';
  const showValues: boolean = config.showValues !== false;
  ...
  const rects = g
    ...
    .attr('x', (d: any) => isHorizontal ? d.y0 : d.x0)
    .attr('y', (d: any) => isHorizontal ? d.x0 : d.y0)
    .attr('width', (d: any) => {
    ...
    .attr('fill', (d: any) => getNodeColor(d, colorScale))
    .attr('rx', 2)
    .attr('ry', 2)
    .attr('stroke', DARK_BG)
    ...
    .on('mouseover', function (event: MouseEvent, d: any) {
      rects.attr('opacity', (n: any) => {
        if (n === d) return 1;
        if (isAncestor(n, d) || isAncestor(d, n)) return 1;
        return 0.3;
      });
      d3.select(this)
        .attr('stroke', '#ffffff')
      ...
      showTooltip(
        tooltip,
        `<strong>${path}</strong><br/>${valueField}: ${formatValue(d.value)}<br/>${pct}% of total`,
        event
      );
    })
  ...
  function zoomTo(target: any) {
    ...
    rects
      ...
      .attr('opacity', (d: any) => {
        if (!isDescendantOrSelf(d, target)) return 0;
        return 1;
      });
...
function positionLabelAt(
  el: any, d: any,
  px: number, py: number, rectW: number, rectH: number,
  isHorizontal: boolean, colorScale: any,
  showValues: boolean, valueField: string
): void {
  const textPadding = 6;
...
function buildHierarchyFromFields(
  data: Record<string, any>[],
  levelFields: string[],
  valueField: string
): any {
  if (!levelFields.length) {
    return d3.hierarchy({ name: 'root', children: [] }).sum((d: any) => d.value || 0);
  }
  ...
  function buildLevel(items: Record<string, any>[], depth: number): any[] {
...
function getNodeColor(d: any, colorScale: any): string {
...
function isAncestor(candidate: any, node: any): boolean {
...
function isDescendantOrSelf(node: any, ancestor: any): boolean {
