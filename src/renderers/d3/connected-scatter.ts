...
export function renderConnectedScatter(container: HTMLElement, spec: VisualizationSpec): void {
  ...
  // ── Scales ──
  const xExtent = d3.extent(parsed, (d: any) => d._x) as [number, number];
  const yExtent = d3.extent(parsed, (d: any) => d._y) as [number, number];
  ...
  if (pointDensity > 5) dotRadius = Math.max(2, baseDotRadius - 2);
  else if (pointDensity > 2) dotRadius = Math.max(3, baseDotRadius - 1);

  // ── Arrow marker definitions ──
  if (showArrows) {
    const defs = svg.append('defs');
    if (hasLegend) {
      const entities = [...new Set(parsed.map((d: any) => d[colorField!]))];
  ...
  // ── Pre-compute positions for all points ──
  ...
  const pointPositions: PointPosition[] = [];
  ...
  if (hasLegend) {
    ...
    entities.forEach((entity: any, entityIdx: number) => {
      const entityData = parsed.filter((d: any) => d[colorField!] === entity);
      ...
      g.selectAll(`.dot-${entityIdx}`)
        ...
        .attr('data-entity', entity)
        .attr('cx', (d: any) => xScale(d._x))
        .attr('cy', (d: any) => yScale(d._y))
        .attr('r', dotRadius)
        .attr('fill', color)
        .attr('stroke', DARK_BG)
      ...
      entityData.forEach((d: any, i: number) => {
        pointPositions.push({
          d,
          cx: xScale(d._x),
          cy: yScale(d._y),
      ...
      if (showLabels) {
        ...
        g.selectAll(`.label-${entityIdx}`)
          ...
          .attr('x', (d: any) => xScale(d._x) + dotRadius + 3)
          .attr('y', (d: any) => yScale(d._y) - dotRadius - 2)
          ...
          .text((d: any, i: number) => {
            ...
            const tooClose = usedPositions.some(
              p => Math.abs(p.x - px) < minDist && Math.abs(p.y - py) < minDist
            );
            if (tooClose) return '';
  ...
  const delaunay = d3.Delaunay.from(
    pointPositions,
    (p: PointPosition) => p.cx,
    (p: PointPosition) => p.cy
  );
