...
export function renderDensityPlot(container: HTMLElement, spec: VisualizationSpec): void {
  ...
  let groups: string[] = [];
  let groupDataMap: Map<string, number[]> = new Map();
  let groupDensities: Map<string, Array<[number, number]>> = new Map();
  let maxDensity = 0;
  ...
  if (categoryField) {
    ...
    groups.forEach((group) => {
      ...
      const localMax = d3.max(density, (d: [number, number]) => d[1]) || 0;
    ...
    maxDensity = d3.max(density, (d: [number, number]) => d[1]) || 0;
  }
  ...
  g.append('rect')
    ...
    .on('mousemove', function (event: MouseEvent) {
      ...
      if (categoryField) {
        ...
        groups.forEach((group) => {
          const density = groupDensities.get(group)!;
          const closest = density.reduce((best, pt) =>
            Math.abs(pt[0] - xVal) < Math.abs(best[0] - xVal) ? pt : best
          );
          const color = colorScale(group);
