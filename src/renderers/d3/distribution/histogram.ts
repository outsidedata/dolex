...
function renderRawHistogram(
  ...
  if (xExtent[0] === xExtent[1]) {
    ...
    const yAxis = g.append('g').attr('class', 'y-axis')
      .call(d3.axisLeft(yScale).ticks(yTickCount).tickSize(-dims.innerWidth).tickPadding(8).tickFormat((d: number) => formatValue(d)));
    styleAxis(yAxis);

    const xAxis = g.append('g').attr('class', 'x-axis').attr('transform', `translate(0,${dims.innerHeight})`)
      .call(d3.axisBottom(d3.scaleLinear().domain([constVal - 1, constVal + 1]).range([0, dims.innerWidth])).ticks(3).tickPadding(8).tickFormat((d: number) => formatValue(d)));
    styleAxis(xAxis);
