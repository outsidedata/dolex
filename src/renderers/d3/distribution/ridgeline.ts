...
export function renderRidgeline(container: HTMLElement, spec: VisualizationSpec): void {
  ...
  const groupKDEs: { group: string; density: DensityPoint[]; values: number[] }[] = [];
  ...
  groups.forEach((group) => {
    ...
    const maxDensity = d3.max(densityPoints, (d: DensityPoint) => d.density) || 0;
  ...
  for (let i = 0; i < groups.length; i++) {
    ...
    g.append('rect')
      ...
      .on('mouseover', function (event: MouseEvent) {
        ...
        showTooltip(
          tooltip,
          tooltipHtml`<strong>${group}</strong><br/>Count: ${groupValues.length}<br/>Median: ${formatValue(med)}<br/>Min: ${formatValue(min)}<br/>Max: ${formatValue(max)}`,
          event
        );
      })
  ...
  for (let i = groups.length - 1; i >= 0; i--) {
    ...
    const heightScale = (d: number) => (d / globalMaxDensity) * rowHeight;
    ...
    const areaGen = d3
      .area()
      .x((d: DensityPoint) => xScale(d.value))
      .y0(yBaseline)
      .y1((d: DensityPoint) => yBaseline - heightScale(d.density))
      .curve(d3.curveBasis);
