...
interface AlluvialFlow {
  ...
  value: number;
  /** Source node y-offset for this flow */
  sy0: number;
  sy1: number;
  /** Target node y-offset for this flow */
  ty0: number;
  ty1: number;
  sourceNode: AlluvialNode;
  targetNode: AlluvialNode;
}

// ─── RENDERER ────────────────────────────────────────────────────────────────

export function renderAlluvial(container: HTMLElement, spec: VisualizationSpec): void {
  const { config, encoding, data } = spec;
  const stageFields: string[] = config.stageFields || [];
  ...
  const colorBy: 'source' | 'target' = config.colorBy || 'source';
  ...
  for (let si = 0; si < stageFields.length; si++) {
    ...
    const nodes: AlluvialNode[] = sorted.map(([category, value]) => ({
      stage: si,
      category,
      value,
      x0: 0,
  ...
  for (let si = 0; si < numStages - 1; si++) {
    ...
    const sourceNodeMap = new Map<string, AlluvialNode>();
    for (const n of stageNodes[si]) sourceNodeMap.set(n.category, n);
    const targetNodeMap = new Map<string, AlluvialNode>();
    for (const n of stageNodes[si + 1]) targetNodeMap.set(n.category, n);

    // Create flow objects
    const stageFlows: AlluvialFlow[] = [];
    ...
    for (const n of stageNodes[si]) sourceOffsets.set(n.category, n.y0);
    ...
    for (const n of stageNodes[si + 1]) targetOffsets.set(n.category, n.y0);
...
function ribbonPath(flow: AlluvialFlow): string {
...
function drawFlows(
  ...
  g.selectAll('.alluvial-flow')
    ...
    .attr('d', ribbonPath)
    .attr('fill', (d: AlluvialFlow) =>
      colorBy === 'target' ? colorScale(d.targetCategory) : colorScale(d.sourceCategory)
    )
    .attr('opacity', flowOpacity)
    .attr('stroke', 'none')
    .on('mouseover', function (event: MouseEvent, d: AlluvialFlow) {
...
function drawNodes(
  ...
  g.selectAll('.alluvial-node')
    ...
    .attr('x', (d: AlluvialNode) => d.x0)
    .attr('y', (d: AlluvialNode) => d.y0)
    .attr('width', (d: AlluvialNode) => d.x1 - d.x0)
    .attr('height', (d: AlluvialNode) => Math.max(d.y1 - d.y0, 3))
    .attr('fill', (d: AlluvialNode) => colorScale(d.category))
    .attr('rx', 2)
    .attr('stroke', DARK_BG)
    .attr('stroke-width', 1)
    .on('mouseover', function (event: MouseEvent, d: AlluvialNode) {
      // Highlight this node
      d3.select(this).attr('opacity', 0.8);

      // Dim unrelated flows, brighten connected ones
      g.selectAll('.alluvial-flow')
        .attr('opacity', (f: AlluvialFlow) => {
          const connected =
            (f.sourceNode.stage === d.stage && f.sourceCategory === d.category) ||
      ...
      const inflows = allFlows.filter(
        f => f.targetNode.stage === d.stage && f.targetCategory === d.category
      );
      const outflows = allFlows.filter(
        f => f.sourceNode.stage === d.stage && f.sourceCategory === d.category
      );
...
function drawLabels(
  ...
  g.selectAll('.alluvial-label')
    ...
    .attr('x', (d: AlluvialNode) => {
      // First stage: label on left; last stage: label on right
      // Middle stages: label on the side with more space
      if (d.stage === 0) return d.x0 - 6;
      if (d.stage === lastStage) return d.x1 + 6;
      // Middle stages: put label on right (flows connect on both sides)
      return d.x1 + 6;
    })
    .attr('y', (d: AlluvialNode) => (d.y0 + d.y1) / 2)
    .attr('dy', '0.35em')
    .attr('text-anchor', (d: AlluvialNode) => (d.stage === 0 ? 'end' : 'start'))
    .attr('fill', TEXT_COLOR)
    .attr('font-size', '10px')
    .attr('font-family', 'Inter, system-ui, sans-serif')
    .text((d: AlluvialNode) => {
      if (d.y1 - d.y0 < 12) return '';
