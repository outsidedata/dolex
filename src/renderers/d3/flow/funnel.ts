...
export function renderFunnel(container: HTMLElement, spec: VisualizationSpec): void {
  ...
  if (data.length < 1 || data.every((d: any) => !d[categoryField] && !d[valueField])) {
    renderEmptyState(
      d3.select(container).append('svg').attr('width', container.clientWidth || 800).attr('height', container.clientHeight || 500).append('g'),
      { innerWidth: (container.clientWidth || 800) - 60, innerHeight: (container.clientHeight || 500) - 60, margin: { top: 30, right: 30, bottom: 30, left: 30 }, width: container.clientWidth || 800, height: container.clientHeight || 500 }
    );
    return;
  }
