...
const WORLD_CITY_COORDS: Record<string, [number, number]> = {
...
export function renderProportionalSymbol(container: HTMLElement, spec: VisualizationSpec): void {
  ...
  const sizeRange: [number, number] = (encoding.size && encoding.size.range) || config.sizeRange || [4, 40];
  ...
  const projection: string = config.projection || 'naturalEarth1';
  ...
  function renderMap(topoData: any): void {
    ...
    const labelField: string = config.labelField || locationField;
    const showRoutes = config.showRoutes === true;
    const routeGroupField: string | null = config.routeGroupField || null;
    const routeStyle = config.routeStyle || {};

    // Place circles for each data point
    const circleData: Array<{
      x: number;
      y: number;
      lon: number;
      lat: number;
    ...
    if (showRoutes && circleData.length >= 2) {
      ...
      if (routeGroupField) {
        ...
        const groups = new Map<string, typeof circleData>();
        ...
        const orderedCircleMap = new Map<any, typeof circleData[0]>();
        circleData.forEach(function (cd) {
        ...
        groups.forEach(function (points, _groupName) {
          if (points.length < 2) { groupIdx++; return; }
          const lineCoords = points.map(function (p) { return [p.lon, p.lat] as [number, number]; });
          const lineGeo = { type: 'LineString' as const, coordinates: lineCoords };
        ...
        const orderedPoints: typeof circleData = [];
        const circleByDatum = new Map<any, typeof circleData[0]>();
        circleData.forEach(function (cd) {
          circleByDatum.set(cd.datum, cd);
        });
        data.forEach(function (d) {
          const cd = circleByDatum.get(d);
          if (cd) orderedPoints.push(cd);
        });

        const lineCoords = orderedPoints.map(function (p) { return [p.lon, p.lat] as [number, number]; });
        const lineGeo = { type: 'LineString' as const, coordinates: lineCoords };
    ...
    if (showLabels) {
      g.selectAll('text.label')
        ...
        .attr('y', (d: any) => d.y + 3)
        .attr('text-anchor', (d: any) => d.x > dims.innerWidth * 0.85 ? 'end' : 'start')
        ...
        .attr('opacity', (d: any) => sizeScale(d.value) < 3 ? 0 : 1)
        .text((d: any) => truncateLabel(String(d.datum[labelField] || ''), 18));
    }
