...
export function renderRadar(container: HTMLElement, spec: VisualizationSpec): void {
  ...
  if (showGrid) {
    for (let level = 1; level <= gridLevels; level++) {
      ...
      const points = dimensions.map((_: string, i: number) => {
  ...
  if (minVal < 0) {
    ...
    const zeroPoints = dimensions.map((_: string, i: number) => {
      const angle = angleSlice * i - Math.PI / 2;
      return [zeroR * Math.cos(angle), zeroR * Math.sin(angle)];
    });
    center.append('polygon')
      .attr('points', zeroPoints.map((p: number[]) => p.join(',')).join(' '))
      .attr('fill', 'none')
      .attr('stroke', '#ffffff')
      .attr('stroke-width', 1)
  ...
  const entityNames = data.map((d: any) => String(d[categoryField]));
  ...
  function getPolygonPoints(row: Record<string, any>): [number, number][] {
  ...
  function highlightEntity(entity: string | null) {
    activeEntity = entity;
    if (entity === null) {
      // Reset all
  ...
  if (hasLegend) {
    const legendDiv = createLegend(colorScale, { shape: 'circle', callbacks: {
      onHover: (entity: string) => highlightEntity(entity),
