...
export function renderScatter(container: HTMLElement, spec: VisualizationSpec): void {
  ...
  // ── Size scale ──
  let sizeScale: (d: any) => number = () => baseRadius;
  if (sizeField) {
    const sizeExtent = d3.extent(validData, (d: any) => Number(d[sizeField])) as [number, number];
  ...
  if (!sizeField) {
    if (pointDensity > 5) dotRadius = Math.max(2, baseRadius - 2);
    else if (pointDensity > 2) dotRadius = Math.max(3, baseRadius - 1);
  }
