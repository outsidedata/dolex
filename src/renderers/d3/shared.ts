...
import type { VisualizationSpec, AxisEncoding, ColorEncoding, ColorPaletteName } from '../../types.js';
import {
  categorical,
  sequential,
  diverging,
...
export function buildXScale(
  encoding: AxisEncoding | undefined,
...
export function buildYScale(
  ...
  switch (encoding.type) {
    ...
    case 'quantitative': {
      ...
      const min = Math.min(0, d3.min(values) as number);
 ...
 */
function resolvePalette(name: ColorPaletteName): string[] | null {
  const palette = PALETTE_MAP[name];
  return palette ? [...palette] : null;
}

function isSequentialPalette(name?: ColorPaletteName): boolean {
  return !!name && name in sequential;
}

function isDivergingPalette(name?: ColorPaletteName): boolean {
  ...
  if (encoding.palette) {
    ...
    if (palette) {
      ...
      if (isDivergingPalette(encoding.palette) && encoding.type === 'quantitative') {
        const values = data.map((d: any) => Number(d[field]));
        const maxAbs = Math.max(Math.abs(d3.min(values) as number), Math.abs(d3.max(values) as number)) || 1;
      ...
      if ((isSequentialPalette(encoding.palette) || isDivergingPalette(encoding.palette)) && valueField) {
        const valueExtent = d3.extent(data, (d: any) => Number(d[valueField])) as [number, number];
        const colorScale = d3
          .scaleLinear()
      ...
      if (encoding.type === 'quantitative') {
        const extent = d3.extent(data, (d: any) => Number(d[field])) as [number, number];
        return d3
          .scaleLinear()
 ...
 */
export function contrastTextMuted(bgColor: string): string {
  const rgb = parseColor(bgColor);
 ...
 */
export function getAdaptiveTickCount(availableSpace: number, minSpacing = 60): number {
 ...
 */
export function calculateLeftMargin(labels: string[], fontSize = 11): number {
 ...
 */
export function truncateLabel(label: string, maxLength = 25): string {
 ...
 */
export function truncateTitle(textEl: any, fullText: string, maxWidth: number): void {
 ...
 */
export function isAllZeros(data: Record<string, any>[], field: string): boolean {
 ...
 */
export function shouldShowValueLabels(
  config: VisualizationSpec['config'],
  barDimension: number,
  isHorizontal: boolean
): boolean {
  if (config.showLabels !== undefined) return config.showLabels;
 ...
 */
export function styleAxis(axis: any): void {
 ...
 */
function fixTickLabels(axis: any): void {
  const ticks = axis.selectAll('.tick text');
  const nodes: any[] = [];
  ticks.each(function (this: any) { nodes.push(this); });
  if (nodes.length < 2) return;

  const texts = nodes.map((n: any) => n.textContent || '');
  const uniqueTexts = new Set(texts);

  if (uniqueTexts.size < Math.min(nodes.length, 3)) {
    const values: number[] = [];
    ticks.each(function (this: any, d: any) { values.push(Number(d)); });
    ...
    if (span > 0) {
      ...
      ticks.each(function (this: any, d: any) {
        const v = Number(d);
        if (!isNaN(v)) this.textContent = fmt(v);
      });
    }

    const reformattedTexts = nodes.map((n: any) => n.textContent || '');
    const seen = new Set<string>();
    reformattedTexts.forEach((t: string, i: number) => {
      if (seen.has(t)) {
...
export function positionTooltip(tooltip: HTMLDivElement, event: MouseEvent): void {
  ...
  if (top + th > vh) top = event.clientY - th - gap;
  if (left < 0) left = 0;
  if (top < 0) top = 0;
...
export function showTooltip(tooltip: HTMLDivElement, html: string, event: MouseEvent): void {
 ...
 */
export function smartTruncateLabels(labels: string[], maxLen: number): string[] {
  if (labels.every(l => l.length <= maxLen)) return labels;
...
function findCommonAffixLength(strings: string[], direction: 'prefix' | 'suffix'): number {
  if (strings.length <= 1) return 0;
  const get = direction === 'prefix'
    ? (s: string, i: number) => s[i]
    : (s: string, i: number) => s[s.length - 1 - i];
...
export function createLegend(
  ...
  if (Array.isArray(source) && source.length > 0 && source[0]?.label !== undefined) {
    entries = source as LegendEntry[];
  } else {
    const domain: string[] = source.domain?.() ?? [];
    entries = domain.map((label: string) => ({
      label: String(label),
      color: source(label),
    }));
  }
  ...
  entries.forEach((entry, idx) => {
    ...
    if (entry.label.length > maxLabelLen) text.title = entry.label;
 ...
 */
export function highlightLegendItem(legendDiv: HTMLDivElement, activeKey: string | null): void {
...
export function renderPlaceholder(container: HTMLElement, spec: VisualizationSpec): void {
...
export function addSortControls(
  ...
  spec: VisualizationSpec,
  dims: ChartDimensions,
  renderFn: (container: HTMLElement, spec: VisualizationSpec) => void
): void {
  ...
  controls.on('click', (event: MouseEvent) => {
    ...
    if (curSortBy === 'value' && curSortOrder === 'descending') {
      newSortBy = 'value'; newSortOrder = 'ascending';
    } else if (curSortBy === 'value' && curSortOrder === 'ascending') {
      newSortBy = 'category'; newSortOrder = 'ascending';
    } else if (curSortBy === 'category' && curSortOrder === 'ascending') {
      newSortBy = 'category'; newSortOrder = 'descending';
    } else {
