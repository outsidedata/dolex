...
export function renderCalendarHeatmap(container: HTMLElement, spec: VisualizationSpec): void {
  ...
  const paletteMap: Record<string, readonly string[]> = {
  ...
  // ── Build day data ──
  const dayData: Array<{ date: Date; key: string; weekCol: number; dayRow: number; value: number }> = [];
  ...
  const monthBoundaries: Array<{ weekCol: number; dayRow: number }> = [];
  ...
  const cells = g.selectAll('.cal-cell')
    ...
    .join('rect')
    .attr('class', 'cal-cell')
    .attr('x', (d: any) => (vertical ? d.dayRow : d.weekCol) * cellStep)
    .attr('y', (d: any) => (vertical ? d.weekCol : d.dayRow) * cellStep)
    .attr('width', cellSize)
    .attr('height', cellSize)
    .attr('rx', cellSize >= 6 ? 2 : 1)
    .attr('fill', (d: any) => {
      if (d.date < minDate! || d.date > maxDate!) return 'transparent';
    ...
    .style('cursor', (d: any) => (d.date >= minDate! && d.date <= maxDate!) ? 'pointer' : 'default');
...
function drawMonthLabelsTop(g: any, startSunday: Date, totalWeeks: number, cellStep: number, cellSize: number, minDate?: Date, maxDate?: Date): void {
  ...
  g.selectAll('.month-label')
    ...
    .attr('x', (d: any) => d.weekCol * cellStep)
    .attr('y', -8)
...
function drawDayLabelsLeft(g: any, cellStep: number, cellSize: number): void {
  ...
  g.selectAll('.day-label')
    ...
    .attr('x', -6)
    .attr('y', (d: number) => d * cellStep + cellSize / 2)
    .attr('text-anchor', 'end')
    .attr('dominant-baseline', 'middle')
    .attr('fill', TEXT_MUTED)
    .attr('font-size', cellSize >= 10 ? '9px' : '7px')
    .attr('font-family', 'Inter, system-ui, sans-serif')
    .text((d: number) => cellSize >= 10 ? DAY_LABELS[d] : DAY_LABELS[d].charAt(0));
}

// ─── VERTICAL LABELS ─────────────────────────────────────────────────────────

/** Day labels across the top (vertical layout) */
function drawDayLabelsTop(g: any, cellStep: number, cellSize: number): void {
  g.selectAll('.day-label')
    .data(DAY_LABELS.map((name, i) => ({ name, i })))
    .join('text')
    .attr('class', 'day-label')
    .attr('x', (d: any) => d.i * cellStep + cellSize / 2)
    .attr('y', -5)
    .attr('text-anchor', 'middle')
    .attr('fill', TEXT_MUTED)
    .attr('font-size', cellSize >= 10 ? '9px' : '7px')
    .attr('font-family', 'Inter, system-ui, sans-serif')
    .text((d: any) => cellSize >= 10 ? d.name : d.name.charAt(0));
}

/** Month labels down the left side (vertical layout) */
function drawMonthLabelsLeft(g: any, startSunday: Date, totalWeeks: number, cellStep: number, minDate?: Date, maxDate?: Date): void {
  const months = getMonthBreaks(startSunday, totalWeeks, minDate, maxDate);
  g.selectAll('.month-label')
    .data(months)
    .join('text')
    .attr('class', 'month-label')
    .attr('x', -8)
    .attr('y', (d: any) => d.weekCol * cellStep + cellStep / 2)
    .attr('text-anchor', 'end')
    .attr('dominant-baseline', 'middle')
    .attr('fill', TEXT_MUTED)
    .attr('font-size', '9px')
...
function getMonthBreaks(startSunday: Date, totalWeeks: number, minDate?: Date, maxDate?: Date): Array<{ name: string; weekCol: number }> {
  const months: Array<{ name: string; weekCol: number }> = [];
  let lastMonth = -1;
  const cursor = new Date(startSunday);
