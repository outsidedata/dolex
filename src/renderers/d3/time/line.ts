...
export function renderLine(container: HTMLElement, spec: VisualizationSpec): void {
  ...
  if (singleDate) {
    xAxisGen.ticks(1).tickFormat(() => (parsedData[0] as Record<string, unknown>)?.[timeField] as string ?? d3.timeFormat('%Y-%m-%d')(xExtent[0]));
  } else {
  ...
  const uniqueDates = new Set(parsedData.map((d: any) => d._date!.getTime()));
  ...
  seriesNames.forEach((name) => {
    ...
    if (showPoints) {
      g.selectAll(`.point-${String(name).replace(/\W/g, '')}`)
        ...
        .attr('cx', (d: any) => xScale(d._date))
        .attr('cy', (d: any) => yScale(d._value))
        .attr('r', 1.25)
        .attr('fill', color)
        .attr('stroke', DARK_BG)
...
function addCrosshairHover(
  ...
  const dateMap = new Map<number, Record<string, any>[]>();
