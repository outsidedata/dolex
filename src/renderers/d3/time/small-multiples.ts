...
export function renderSmallMultiples(container: HTMLElement, spec: VisualizationSpec): void {
  ...
  const allDates = data.map((d) => parseDate(d[timeField])).filter(Boolean) as Date[];
  const allValues = data.map((d) => Number(d[valueField])).filter((v) => !isNaN(v));
  const xDomain = d3.extent(allDates) as [Date, Date];
  const yDomain = [Math.min(0, d3.min(allValues) as number), d3.max(allValues) as number];
