...
export function renderStreamGraph(container: HTMLElement, spec: VisualizationSpec): void {
  ...
  const offsetMode: string = config.offset || 'wiggle';
  const curveMode: string = config.curve || 'basis';
  const interactive = config.interactive !== false;

  if (!timeField || !valueField || !seriesField) return;
  ...
  const seriesNames: string[] = [...new Set(parsedData.map((d: any) => d._series))] as string[];
  ...
  const offsetMap: Record<string, any> = {
  ...
  const hasNegativeValues = wideData.some((row: any) =>
    seriesNames.some((name: string) => row[name] < 0)
  );
  ...
  const yMin = d3.min(stacked, (layer: any) => d3.min(layer, (d: any) => d[0])) as number;
  const yMax = d3.max(stacked, (layer: any) => d3.max(layer, (d: any) => d[1])) as number;
...
function addStreamInteraction(
  ...
  streams
    ...
    .on('mouseover', function (_event: MouseEvent, d: any) {
      ...
      streams
        .attr('opacity', (s: any) => (s.key === hoveredKey ? 1 : 0.2))
        .attr('stroke-width', (s: any) => (s.key === hoveredKey ? 1.5 : 0.5));
    ...
    .on('mousemove', function (event: MouseEvent, d: any) {
      ...
      const layer = stacked.find((l: any) => l.key === hoveredKey);
      if (layer) {
        const pt = layer[idx];
