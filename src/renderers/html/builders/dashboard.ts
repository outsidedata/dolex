...
export function buildDashboardHtml(
  ...
  const viewHtmlEntries: { viewId: string; chartHtml: string; title: string }[] = [];
  ...
  const chartHtmlMapJson = safeJson(
    Object.fromEntries(viewHtmlEntries.map(e => [e.viewId, e.chartHtml])),
  );
...
function buildViewSizeCss(
  spec: DashboardSpec,
  entries: { viewId: string }[],
): string[] {
  ...
  return entries.map(e => {
    ...
    if (!s) return '';
    const parts: string[] = [];
    if (s.colSpan) parts.push(`grid-column: span ${s.colSpan}`);
    if (s.rowSpan) parts.push(`grid-row: span ${s.rowSpan}`);
