...
export function shouldCompound(
  ...
  // Explicit opt-out
  if (options?.compound === false) return false;

  // Skip patterns where table doesn't help
  if (SKIP_TABLE_PATTERNS.has(spec.pattern)) return false;

  // Skip if too many rows
  if (spec.data.length > MAX_COMPOUND_ROWS) return false;

  // Skip if no data
  if (!spec.data.length) return false;
 ...
 */
export function buildCompoundSpec(
  spec: VisualizationSpec,
  columns?: DataColumn[]
): CompoundVisualizationSpec {
      ...
      },
      {
        ...
        table: {
          ...
          sort: interactionField
            ? { field: interactionField, direction: 'asc' as const }
            : undefined,
          pageSize: 100,
        },
      },
 ...
 */
function findInteractionField(spec: VisualizationSpec): string | undefined {
...
function buildTableColumns(
  ...
  const encodingFields = new Set<string>();

  // Collect fields used in encoding
  if (spec.encoding.x?.field) encodingFields.add(spec.encoding.x.field);
  if (spec.encoding.y?.field) encodingFields.add(spec.encoding.y.field);
  if (spec.encoding.color?.field) encodingFields.add(spec.encoding.color.field);
  if (spec.encoding.size?.field) encodingFields.add(spec.encoding.size.field);
  ...
  if (dataColumns) {
    dataColumns.forEach(c => { if (c.type === 'numeric') numericFields.add(c.name); });
  } else {
    ordered.forEach(field => {
      ...
      if (numCount > sample.length * 0.7) numericFields.add(field);
    });
  }
