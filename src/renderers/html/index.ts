...
const builders: Record<string, (spec: VisualizationSpec) => string> = {
  'bar': buildBarHtml,
 ...
 */
export function buildChartHtml(spec: VisualizationSpec): string {
  if (!spec || !spec.pattern) {
    return buildHtml(
      { pattern: 'error', title: 'Error', data: [], encoding: {}, config: {} },
 ...
 */
export function getSupportedHtmlPatterns(): string[] {
 ...
 */
export function isHtmlPatternSupported(pattern: string): boolean {
  return pattern in builders;
}

// ─── PLACEHOLDER ────────────────────────────────────────────────────────────

function buildPlaceholderHtml(spec: VisualizationSpec): string {
...
export type { DashboardViewData } from './builders/dashboard.js';
