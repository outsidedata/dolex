...
function walk(node: AstNode, refs: Set<string>): void {
  switch (node.type) {
    case 'column': refs.add(node.name); break;
    case 'binary': walk(node.left, refs); walk(node.right, refs); break;
    case 'unary': walk(node.operand, refs); break;
    case 'call': for (const a of node.args) walk(a, refs); break;
    case 'array': for (const e of node.elements) walk(e, refs); break;
  }
}

/** Build a dependency map: column → columns it references. */
export function buildDependencyMap(records: TransformRecord[]): Map<string, Set<string>> {
...
export function findDependents(column: string, records: TransformRecord[]): string[] {
  const depMap = buildDependencyMap(records);
  const dependents = new Set<string>();

  function findRecursive(col: string): void {
...
export function hasCircularDependency(
  newColumn: string,
  newExpr: string,
  existingRecords: TransformRecord[]
): { circular: boolean; cycle?: string[] } {
  const allRecords = [
    ...existingRecords,
    { column: newColumn, expr: newExpr, type: 'numeric' as const, layer: 'working' as const, order: 0 },
  ];
  ...
  function dfs(col: string): string[] | null {
    if (inStack.has(col)) {
      const cycleStart = path.indexOf(col);
      return [...path.slice(cycleStart), col];
    }
    if (visited.has(col)) return null;
...
export function topologicalSort(records: TransformRecord[]): TransformRecord[] {
  const depMap = buildDependencyMap(records);
  ...
  function visit(col: string): void {
