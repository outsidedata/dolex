...
export interface DataColumn {
  ...
  nullCount: number;
  totalCount: number;
  // ─── RICH PROFILING (populated by connectors) ──────────────────────
...
export function isDslAggregateField(field: DslSelectField): field is DslAggregateField {
  return typeof field !== 'string' && 'aggregate' in field;
}

/** Type guard: check if a select field is a window function field */
export function isDslWindowField(field: DslSelectField): field is DslWindowField {
...
export function isCompoundSpec(
  spec: VisualizationSpec | CompoundVisualizationSpec
): spec is CompoundVisualizationSpec {
  return 'compound' in spec && (spec as CompoundVisualizationSpec).compound === true;
}
...
export interface DashboardLayout {
  ...
  viewSizes?: Record<string, { colSpan?: number; rowSpan?: number }>;
}
...
export function isDashboardSpec(
  spec: VisualizationSpec | CompoundVisualizationSpec | DashboardSpec
): spec is DashboardSpec {
  return 'dashboard' in spec && (spec as DashboardSpec).dashboard === true;
}
