...
import { OPTIONAL_FEATURES, type OptionalFeatureKey } from './optional-deps.js';
import { packagePath } from './package-root.js';

const require = createRequire(import.meta.url);

/** A package's version, resolved without loading it. `null` ⇒ not installed. */
function pkgVersion(spec: string): string | null {
  try { return JSON.parse(readFileSync(require.resolve(`${spec}/package.json`), 'utf8')).version ?? null; }
  catch { return null; }
}
...
export interface CapabilitiesReport {
  ...
  sources: { csv: string; postgres: string; mongodb: string };
  deps: CapabilityDep[];
  python: { available: boolean; version?: string; enables: string; install?: string };
}

const PROBES: { key: OptionalFeatureKey; spec: string; enables: string }[] = [
...
export function probeCapabilities(): CapabilitiesReport {
  const coreOk = !!pkgVersion('better-sqlite3');

  const deps: CapabilityDep[] = PROBES.map((p) => {
  ...
  const has = (spec: string) => deps.find((d) => d.package === spec)!.installed;

  let pyVersion: string | undefined;
  try { pyVersion = execFileSync('python3', ['--version'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim(); } catch { /* absent */ }
  ...
  let dolexVersion: string | undefined;
  try { dolexVersion = JSON.parse(readFileSync(packagePath('package.json'), 'utf8')).version; } catch { /* best-effort */ }
