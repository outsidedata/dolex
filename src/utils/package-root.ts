...
export function findPackageRoot(fromUrl: string): string {
  ...
  let dir = startDir;
  let firstWithPkg: string | undefined;
  for (let i = 0; i < 12; i++) {
    const pkg = join(dir, 'package.json');
    if (existsSync(pkg)) {
      if (firstWithPkg === undefined) firstWithPkg = dir;
      try {
        const name = JSON.parse(readFileSync(pkg, 'utf-8')).name;
        if (PKG_NAMES.has(name)) return dir;
      } catch {
...
export function packagePath(fromUrl: string, ...segments: string[]): string {
