 ...
 */
export function getContainerMode(width: number, height?: number): ContainerMode {
 ...
 */
export function responsiveMargins(
  width: number,
  height: number,
  options?: {
    hasTitle?: boolean;
  ...
  const base: Record<ContainerMode, ChartMargins> = {
  ...
  if (opts.hasTitle) m.top += mode === 'compact' ? 8 : 12;
  if (opts.hasXLabel) m.bottom += mode === 'compact' ? 10 : 16;
  if (opts.hasYLabel) m.left += mode === 'compact' ? 10 : 16;
  if (opts.hasLegend) m.right += mode === 'compact' ? 40 : 80;
...
export function responsiveFontSize(
  width: number,
  role: 'title' | 'axisLabel' | 'tickLabel' | 'annotation' | 'legend' = 'tickLabel'
): number {
  const mode = getContainerMode(width);

  const sizes: Record<typeof role, Record<ContainerMode, number>> = {
 ...
 */
export function responsiveTicks(axisLength: number, isCategory = false): number {
  if (isCategory) {
